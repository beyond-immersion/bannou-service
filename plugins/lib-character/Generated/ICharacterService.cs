//----------------------
// <auto-generated>
//     Generated from OpenAPI schema: schemas/character-api.yaml
//
//     WARNING: DO NOT EDIT THIS FILE (TENETS T1/T2)
//     This file is auto-generated from the API schema.
//     Any manual changes will be overwritten on next generation.
//
//     To modify the interface:
//     1. Edit the source schema (schemas/character-api.yaml)
//     2. Run: scripts/generate-all-services.sh
//     3. Implement methods in CharacterService.cs
//
//     See: docs/reference/tenets/FOUNDATION.md (T1: Schema-First, T2: Code Generation)
// </auto-generated>
//----------------------

using BeyondImmersion.BannouService;
using BeyondImmersion.BannouService.Services;
using System.Threading;
using System.Threading.Tasks;

#nullable enable

namespace BeyondImmersion.BannouService.Character;

/// <summary>
/// Service interface for Character API
/// </summary>
public partial interface ICharacterService : IBannouService
{
        /// <summary>
        /// CreateCharacter operation
        /// </summary>
        Task<(StatusCodes, CharacterResponse?)> CreateCharacterAsync(CreateCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// GetCharacter operation
        /// </summary>
        Task<(StatusCodes, CharacterResponse?)> GetCharacterAsync(GetCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// UpdateCharacter operation
        /// </summary>
        Task<(StatusCodes, CharacterResponse?)> UpdateCharacterAsync(UpdateCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// DeleteCharacter operation
        /// </summary>
        Task<StatusCodes> DeleteCharacterAsync(DeleteCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// ListCharacters operation
        /// </summary>
        Task<(StatusCodes, CharacterListResponse?)> ListCharactersAsync(ListCharactersRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// GetEnrichedCharacter operation
        /// </summary>
        Task<(StatusCodes, EnrichedCharacterResponse?)> GetEnrichedCharacterAsync(GetEnrichedCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// CompressCharacter operation
        /// </summary>
        Task<(StatusCodes, CharacterArchive?)> CompressCharacterAsync(CompressCharacterRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// GetCharacterArchive operation
        /// </summary>
        Task<(StatusCodes, CharacterArchive?)> GetCharacterArchiveAsync(GetCharacterArchiveRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// CheckCharacterReferences operation
        /// </summary>
        Task<(StatusCodes, CharacterRefCount?)> CheckCharacterReferencesAsync(CheckReferencesRequest body, CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// GetCharactersByRealm operation
        /// </summary>
        Task<(StatusCodes, CharacterListResponse?)> GetCharactersByRealmAsync(GetCharactersByRealmRequest body, CancellationToken cancellationToken = default(CancellationToken));

}
