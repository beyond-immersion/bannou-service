...
public class MinioStorageProvider : StorageModels.IAssetStorageProvider
{
    ...
    /// Replaces internal endpoint (e.g., minio:9000) with public endpoint and upgrades to HTTPS.
    /// </summary>
    ...
    private string RewriteUrlForPublicAccess(string url)
    {
        ...
        // Replace internal endpoint with public endpoint and upgrade to HTTPS
        // MinIO generates URLs like: http://minio:9000/bucket/key?signature
        // We rewrite to: https://public.domain:9000/bucket/key?signature
        var result = url.Replace($"http://{_options.Endpoint}", $"https://{_options.PublicEndpoint}");

        // Also handle case where MinIO was configured with HTTPS internally
        result = result.Replace($"https://{_options.Endpoint}", $"https://{_options.PublicEndpoint}");
