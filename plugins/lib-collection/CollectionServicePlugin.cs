...
/// </summary>
public class CollectionServicePlugin : BaseBannouPlugin
{
    ...
    public override void ConfigureServices(IServiceCollection services)
    {
        ...
        // No need to register ICollectionService and CollectionService here

        // Configuration registration is now handled centrally by PluginLoader based on [ServiceConfiguration] attributes
        // No need to register CollectionServiceConfiguration here
    ...
    {
        Logger?.LogInformation("Configuring Collection service application pipeline");

        // The generated CollectionController should already be discovered via standard ASP.NET Core controller discovery

        // Store service provider for lifecycle management
        _serviceProvider = app.Services;

        Logger?.LogInformation("Collection service application pipeline configured");
    }
    ...
    {
        Logger?.LogInformation("Starting Collection service");
        ...
        try
        {
            ...
            _service = _scope?.ServiceProvider.GetService<ICollectionService>();

            if (_service == null)
            {
                Logger?.LogError("Failed to resolve ICollectionService from DI container");
                return false;
            }
            ...
            {
                Logger?.LogDebug("Calling IBannouService.OnStartAsync for Collection service");
                await bannouService.OnStartAsync(CancellationToken.None);
            }

            Logger?.LogInformation("Collection service started successfully");
            return true;
        }
        catch (Exception ex)
        {
            Logger?.LogError(ex, "Failed to start Collection service");
            return false;
        }
    }
    ...
    protected override async Task OnRunningAsync()
    {
        ...
        Logger?.LogDebug("Collection service running");
        ...
        try
        {
            ...
            if (_service is IBannouService bannouService)
            {
                Logger?.LogDebug("Calling IBannouService.OnRunningAsync for Collection service");
                await bannouService.OnRunningAsync(CancellationToken.None);
            }
        }
        catch (Exception ex)
        {
            Logger?.LogWarning(ex, "Exception during Collection service running phase");
        }
    }
    ...
    protected override async Task OnShutdownAsync()
    {
        ...
        Logger?.LogInformation("Shutting down Collection service");
        ...
        try
        {
            ...
            if (_service is IBannouService bannouService)
            {
                Logger?.LogDebug("Calling IBannouService.OnShutdownAsync for Collection service");
                await bannouService.OnShutdownAsync();
            }

            Logger?.LogInformation("Collection service shutdown complete");
        }
        catch (Exception ex)
        {
            Logger?.LogWarning(ex, "Exception during Collection service shutdown");
        }
        finally
        {
            _scope?.Dispose();
