    ...
    if echo "$command" | grep -qE "($code_extensions)"; then
        jq -n '{
            hookSpecificOutput: {
                ...
                permissionDecisionReason: "BLOCKED: Moving/renaming code files is FORBIDDEN without explicit user instruction.\n\nAn agent previously renamed files to .tmp instead of fixing build errors.\n\nPer CLAUDE.md FORBIDDEN DESTRUCTIVE COMMANDS:\n- mv (for code files) - Can lose files or break references\n\nIF THE BUILD IS FAILING: FIX THE CODE, do not rename files.\nIF YOU TRULY NEED TO MOVE A FILE: STOP and ask the user first."
            }
        }'
        exit 0
    fi
