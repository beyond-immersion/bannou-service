...
|---|------|---------|---------------|----------|--------|
| 1 | lib-auth/Services/OAuthProviderService.cs:769 | Provider -> OAuthProvider | **V2** (subset) | High | **DONE** — Both enums consolidated to common-api.yaml. Provider removed. Identity mapping deleted. |
| 2 | lib-account/AccountService.cs:797 | OAuthProvider -> AuthProvider | **V2** (superset) | High | **DONE** — Both enums consolidated to common-api.yaml. No mapping needed. |
...
| 7 | lib-documentation/DocumentationService.cs:3092-3153 | BindingStatusInternal <-> BindingStatus, SyncStatusInternal <-> SyncStatus, OwnerTypeInternal <-> DocumentationOwnerType | **V4** (internal duplicates) | High | **DONE** — 3 internal enums deleted, 6 identity mapping methods removed, API enums used directly. |
...
| 9 | lib-messaging/RabbitMQMessageSubscriber.cs:226 | ExchangeType -> RmqExchangeType | **A1** (third-party) | None | **DONE** — Code-only SubscriptionExchangeType deleted. Schema-generated ExchangeType used in all 14+ call sites. A1 boundary mapping to RmqExchangeType preserved. Drift test added (EnumMappingValidator.AssertSwitchCoversAllValues). |
...
| 22 | lib-save-load/SaveLoadService.cs:2476 | string -> SaveCategory, CompressionType | ~~V5~~ **DONE** | ~~Medium~~ | **DONE**: Replaced `DefaultCompressionByCategory` string with 5 typed `$ref` properties in save-load-configuration.yaml. `GetDefaultCompressionType` now uses typed switch over SaveCategory returning config properties directly. No string parsing remains. |
...
| **High** | Auth/Account provider enum consolidation to common-api.yaml | V1/V2 | Medium (schema + regen + update both services) | **DONE** — OAuthProvider/AuthProvider in common-api.yaml, Provider removed, lifecycle ref resolver created, filtered schema handling fixed |
| **High** | Documentation: remove internal enum duplicates, use API enums | V4 | Small (code cleanup only) | **DONE** — 3 internal enums deleted, 6 identity mapping methods removed, API enums used directly |
| **Medium** | Save-Load: replace string config with typed enum properties | V5 | Small (schema + config + regen) | **DONE** — 5 typed CompressionType config properties, Enum.TryParse eliminated |
| **Medium** | Messaging: unify SubscriptionExchangeType to schema-generated ExchangeType | V1 (code-only duplicate) | Medium (14+ call sites across 4 plugins) | **DONE** — Code-only enum removed, schema ExchangeType used, drift test added |
