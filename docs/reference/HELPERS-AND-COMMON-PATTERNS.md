...
### Action + Lifecycle Dual Publishing

**Rule** (FOUNDATION TENETS T5 "Action events vs. lifecycle events"): every user/service-action mutation on an `x-lifecycle` entity MUST publish **both**:

1. The **semantic action event** (for game-logic consumers — e.g., `quest.completed`, `save-slot.committed`, `craft.session.started`)
2. The **lifecycle `*.updated` event** (for infrastructure consumers — caches, analytics, downstream projections) with `changedFields` naming which fields changed

Background/system-driven changes (TTL expiration, decay, migration) publish only the `*.updated` event — there is no action to announce. The decision test is **"did something *happen*, or did state just *change*?"** (see FOUNDATION.md § T5).

**Canonical pattern** — action handler publishes both events at the same commit point:

```csharp
// Save the mutated state first
await _saveSlotStore.SaveAsync(BuildSaveSlotKey(slotId), slot, cancellationToken: ct);

// 1. Action event — game-logic consumers react to the semantic operation
await _messageBus.PublishSaveSlotCommittedAsync(new SaveSlotCommittedEvent
{
    EventId = Guid.NewGuid(),
    Timestamp = DateTimeOffset.UtcNow,
    SlotId = slotId,
    SaveId = saveId,
    // ... semantic fields specific to "commit" ...
}, ct);

// 2. Lifecycle updated — infrastructure consumers react to the state change
await _messageBus.PublishSaveSlotUpdatedAsync(slot,
    changedFields: new[] { "latestSaveId", "saveCount", "updatedAt" });
```

**Schema requirement**: `x-event-publications` declares BOTH topics. Lifecycle `*.updated` is generated automatically from `x-lifecycle` unless the entity is marked `immutable: true`. When every mutation path is an action, the `*.updated` publication must still be declared alongside — not omitted — so the generator produces the publisher the dual-publish pattern requires.

**`immutable: true` is the opposite escape hatch**: use it only when an entity genuinely has no mutation path (e.g., immutable historical records, write-once audit logs). Marking an entity immutable suppresses `*UpdatedEvent` generation; pairing it with any mutation at all is a contradiction. Structural test `LifecycleEntities_ImmutableFlagMatchesMutationSurface` enforces the bidirectional invariant: the flag is present iff no `*.updated` publication exists.

**When to apply**: every mutation endpoint on an entity with `x-lifecycle` (create is already handled by the `*Created` lifecycle event; delete by `*Deleted`). The rule covers the middle — any action that modifies an existing entity.
