...
plugins/lib-{service}/
├── Generated/                       # NEVER EDIT - regenerated by the generate tool
...
> **Model Inspection**: Use the `print_models` MCP tool to view compact model shapes instead of reading generated files directly. If it fails, regenerate first — never guess at model definitions.
...
4. **Regenerate after schema changes** — use the `generate` MCP tool (run `generate()` with no args to see all available generators, triggers, and ordering rules), then verify the build passes
...
| Compound entity name without kebab-case in topic (`clausetype`, `templatevalues`) | T16 | Use kebab-case: `contract.clausetype.registered` → `contract.clause-type.registered` |
