...
10. **No Structured Data as Parseable Strings**: Configuration properties that represent enum values or structured data must use typed schema constructs. Comma-delimited strings parsed at runtime bypass all T21 guarantees — no compile-time type safety, no schema validation, no generated config class typing. When per-category properties are needed, define individual typed properties with `$ref` to the enum type.

```yaml
# FORBIDDEN: Parseable string bag requiring runtime splitting and Enum.Parse
DefaultCompressionByCategory:
  type: string
  nullable: true
  env: SAVE_LOAD_DEFAULT_COMPRESSION_BY_CATEGORY
  description: "QUICK_SAVE:NONE,AUTO_SAVE:GZIP,..."

# CORRECT: Individual typed properties — compile-time safe, zero parsing
DefaultCompressionQuickSave:
  $ref: 'save-load-api.yaml#/components/schemas/CompressionType'
  env: SAVE_LOAD_DEFAULT_COMPRESSION_QUICK_SAVE
  default: None
DefaultCompressionAutoSave:
  $ref: 'save-load-api.yaml#/components/schemas/CompressionType'
  env: SAVE_LOAD_DEFAULT_COMPRESSION_AUTO_SAVE
  default: Gzip
```
...
3. **Internal POCOs**: MUST use the same generated enum types from request/response/event models — not string, not a hand-written duplicate enum. If `ItemRarity` is generated by NSwag, your `ItemModel` storage class uses `ItemRarity`, not `string` and not a manually-defined `ItemRarityInternal`. Duplicating a generated enum in service code is a V4 violation (see SCHEMA-RULES.md § "Enum Boundary Classification").
...
5. **Authored Content Boundary (A2 SDK Boundary)**: Plugin SDKs (MusicTheory, StorylineTheory, ABML parser) define their own enum types independently from OpenAPI schemas. At the boundary where plugin code meets generated models, string-to-enum or enum-to-enum mapping is acceptable because the SDK is a separate compilation unit with its own type system.

```csharp
// ACCEPTABLE: SDK enum mapped at plugin boundary (A2 boundary)
