...
**Ephemeral instance entities**: When a Category B template's instances are ephemeral (sessions, tickets, or other transient entities that complete/expire/cancel rather than persist indefinitely), the clean-deprecated sweep's `hasActiveInstancesAsync` delegate simply checks for any currently-active instances rather than a persistent reverse index. Since ephemeral instances self-resolve (they complete, time out, or are cancelled within a bounded window), the sweep can treat any deprecated template with zero active instances as eligible for cleanup. The `instanceEntity` declaration is still required (the instance entity must still be an x-lifecycle entity in the same events file), but the implementation may use a direct store query (e.g., "any active sessions referencing this template?") instead of a maintained reverse index. This is simpler than the persistent-instance case because the count converges to zero naturally — no instance deletion is required to reach eligibility.
