...
**Action events vs. lifecycle events**: When a **user or service action** causes a state change, publish **both** the semantic action event (for game logic consumers) and the lifecycle `*.updated` event (for infrastructure consumers). Example: completing a quest publishes `quest.completed` (action — Achievement reacts) and `quest.instance.updated` (lifecycle — caches invalidate, analytics ingest). When a **background process or system mechanism** changes state without an explicit action, publish **only** the lifecycle `*.updated` event with `changedFields`. There is no action to announce — the state simply changed. Examples: rating period decay increases RD (publish `analytics.rating.updated` with `MatchId = null`), subscription expiration changes status (publish `subscription.updated`), deprecation marks an entity (publish `*.updated` with deprecation changedFields per T31). **The test**: "Did something *happen*, or did state just *change*?" If something happened (a match was played, a quest was completed, a milestone was reached), there is a semantic action event alongside the lifecycle event. If state drifted due to time, background computation, or system mechanics, there is only a lifecycle event.
