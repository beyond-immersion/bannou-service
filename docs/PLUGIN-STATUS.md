...
## FAQ: Why Is There No Combat / Skill / Magic Plugin? {#faq-composite-systems}

> **Who this is for**: Agents evaluating "missing" plugins during design phases, and humans wondering why the plugin list has no dedicated combat system, skill system, magic system, class system, or similar gameplay feature plugin.

### The Short Answer

Combat, skills, magic, classes, and similar gameplay systems are **not plugins** in Bannou. They are **emergent compositions** of existing primitives. There is no "missing" combat plugin, skill plugin, magic plugin, or class plugin — these concepts are behavioral outcomes that arise from the interaction of services that already exist.

### The Longer Answer

Traditional game backends treat combat, skills, and magic as discrete systems with dedicated databases, APIs, and business logic. Bannou rejects this approach because these concepts are not *things* — they are *behaviors* that emerge from the interaction of orthogonal primitives.
...
| Obligation constraints | **Obligation** — "should I cast this forbidden spell?" cost modifiers | L4 |

No single plugin "owns" magic. A character is a mage because nine independent systems converge to make them behave like one. The same decomposition applies to every composite concept:

**"A warrior"** = high `${nature.aggression}` (Ethology) + "combat" seed growth (Seed) + weapon mastery board (License) + combat buffs (Status) + melee-focused behavior documents (Actor) + aggressive combat preferences (Character-Personality) + martial heritage aptitudes (Character-Lifecycle)

**"A skill system"** = License boards (progression UI) + Seed growth (proficiency accumulation) + Collection unlocks (skill grants) + Status effects (passive bonuses) + Actor behavior (skill usage decisions)

**"Combat"** = Actor behavior execution (decisions) + Character-Personality `${combat.*}` variables (preferences) + Ethology `${nature.*}` variables (instincts) + Status effects (active buffs/debuffs) + Mapping affordances (spatial awareness) + Seed capabilities (what actions are available) + Obligation cost modifiers (moral hesitation)

### Why This Matters for Design

If you are designing a new feature and find yourself thinking "we need a combat plugin" or "we need a skill plugin," stop and decompose the concept:

1. **What data does it store?** → Probably already covered by Seed (growth), License (progression), Status (effects), Character-Personality (preferences), or Collection (unlocks)
2. **What decisions does it make?** → That's Actor behavior, expressed in ABML documents that read from variable providers
3. **What effects does it produce?** → Status is the unified effects query layer — all buffs, debuffs, blessings, curses, and passive bonuses flow through it
4. **What progression does it track?** → License for grid-based boards, Seed for organic growth, Collection for unlock archives

The correct response to "we need a magic system" is not a new plugin — it's a new set of Seed types, License boards, Status templates, ABML behavior documents, and variable provider data that compose into the experience of magic.

### The Orchestration Layer Pattern

Some gameplay concepts *do* warrant their own plugin — but only when they need to **orchestrate** existing primitives in a specific way that no existing service handles. These are "thin orchestration layers":

| Orchestrator | What It Composes | Why It Exists |
|-------------|------------------|---------------|
...
If your proposed plugin is a thin orchestration layer that composes existing primitives into a new player-facing experience — and no existing orchestrator already covers the use case — then it may be warranted. But if it's trying to *own* a concept that's inherently composite (combat, skills, magic, classes), it's architecturally wrong.
...
2. **Does the concept need its own state that no existing store covers?** If everything lives in Seed/License/Status/Collection already, there's no new state to manage.
3. **Does the concept need orchestration logic that no existing orchestrator provides?** Quest, Escrow, License, and Divine already cover most multi-primitive coordination patterns.
4. **Would the plugin just be a query aggregator?** Status already aggregates effects from all sources. Adding another aggregator creates confusion about which is authoritative.
