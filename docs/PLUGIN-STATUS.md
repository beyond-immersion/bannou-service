...
| [Storyline](#storyline-status) | L4 | 92% | 0 | **Hardened (2x).** All 16 endpoints. Lifecycle events wired (created/updated/deleted), IEventConsumer, typed publishers, CleanDeprecated via DeprecationCleanupHelper, null-forgiving removed, ?? string.Empty validated, ApiException catch removed, RiskSeverity enum extracted, NRT Rule 4, maxLength/minimum constraints, 68 unit tests. 3 design decisions deferred (T26 Guid.Empty, T7 compensation, T29 additionalProperties). |
...
| [Utility](#utility-status) | L4 | 0% | 0 | Pre-implementation. L4-audited (2026-03-11): Workshop→soft dep, 6 string→enum types, location cleanup→x-references, Category B deprecation lifecycle, dead deps removed (Contract/Currency), ITelemetryProvider, worker startup delays, per-item error isolation, testing strategy, topic_prefix/x-lifecycle notes. 6 design decisions deferred (#628-#633). No schema, no code. |
| [Workshop](#workshop-status) | L4 | 0% | 0 | Pre-implementation. Deep dive hardened (2026-03-11): T31 Category B deprecation, T25 type safety, T26 no sentinels, T16 Pattern C topics, T28 account cleanup, T7 compensation deferred (GH #626). 9 GH issues tracked. No schema, no code. |
...
**Hardened 2x (2026-03-11).** All 16 endpoints implemented across composition (compose, get plan, list plans, delete plan), scenario definitions (CRUD, deprecate, list with filtering, clean-deprecated), scenario discovery (find available, test trigger, evaluate fit), and scenario execution (trigger with mutations/quest hooks, get active, get history, get compress data). Wraps `storyline-theory` and `storyline-storyteller` SDKs for seeded narrative generation from compressed archives.

**Audit fixes (pass 2, 2026-03-11)**: Lifecycle events wired — `CreateScenarioDefinitionAsync` publishes `storyline.scenario-definition.created`, `UpdateScenarioDefinitionAsync` publishes `storyline.scenario-definition.updated` with `changedFields`. Constructor now accepts `IEventConsumer` and calls `RegisterEventConsumers`. All `TryPublishAsync` calls replaced with generated typed publishers (`PublishScenarioTriggeredAsync`, `PublishScenarioCompletedAsync`, `PublishStorylinePlanComposedAsync`). `CleanDeprecatedScenarioDefinitionsAsync` implemented using `DeprecationCleanupHelper.ExecuteCleanupSweepAsync`. Null-forgiving operators removed from `ComputeCacheKey` (`.Value` → `.GetValueOrDefault()`). `?? string.Empty` replaced with validation in backstory mutation path. Unnecessary `ApiException` catch removed from `GetCompressDataAsync` (no inter-service calls). Schema: inline `StorylineRiskSeverity` enum extracted to standalone `RiskSeverity`, `$ref` format fixed (`./common-api.yaml` → `common-api.yaml`), `operationId` PascalCase, NRT Rule 4 (value types in required arrays), `maxLength`/`minimum` validation constraints on 30+ properties. 68 unit tests passing (7 new key builder format stability tests).

**Audit fixes (pass 1, 2026-03-09)**: Build*Key() pattern for all state store keys. Inline topic strings replaced with `StorylinePublishedTopics` constants; lifecycle event names fixed (no service prefix). try-catch narrowed to SDK call sites + ApiException for inter-service calls. `Found` filler boolean removed from GetPlanResponse and GetScenarioDefinitionResponse. Enum `.ToString()` removed from event publishing; ScenarioMutation enums changed from `type: string` to Storyline-owned enums with A2 boundary `MapByName` mapping. Topic rename `storyline.composed` → `storyline.plan.composed`. Dead config removed. Category B deprecation lifecycle. x-permissions corrected. ServiceConstructorValidator test added.

**Remaining gaps**: Iterative composition (ContinuePhase) not exposed via HTTP. EntitiesToSpawn always null. No event subscriptions for content flywheel integration (resource.compressed). 3 design decisions deferred to deep dive (T26 Guid.Empty sentinel, T7 multi-service compensation, T29 additionalProperties patterns).
...
Pre-implementation spec hardened via L4 audit (2026-03-11). Critical fixes: Workshop dependency L4→soft with manual-rate fallback (#628), 6 string fields converted to typed enums (ConnectionStatus, ProductionSourceStatus, MaintenanceRequestStatus, MaintenanceType, CoverageStatus, EntityType for ownerType), location.deleted subscription replaced with lib-resource x-references (CASCADE), IResourceClient added as L1 hard dependency, Category B deprecation lifecycle added for UtilityNetworkType (deprecate, clean-deprecated, instance guard, no per-entity delete). Warning fixes: dead IContractClient/ICurrencyClient removed, tags documented as non-system-state, utility.maintenance.cancelled event added, ITelemetryProvider in DI, worker startup delay configs, debounce multi-instance safety documented, per-item error isolation in workers, testing strategy section, topic_prefix/x-lifecycle notes, decay timing clarified (real-time interval with game-time calculation), state-stores.yaml note. 6 design decisions deferred: #628 (Workshop mode), #629 (junction fairness), #630 (coverage ratio), #631 (demolition semantics), #632 (disabledReason type), #633 (debounce implementation). 25 planned endpoints, 5 state stores, 16 published events, 3 consumed events, 2 background workers, 13 configuration properties. No schema, no code.
...
| # | Enhancement | Description | Issue |
|---|-------------|-------------|-------|
| 1 | **Schema creation and initial implementation** | Full 7-phase implementation plan in deep dive. Phases 1-3 cover network type registry, connection graph, and flow calculation engine. | — |
| 2 | **IUtilityFlowModifierProvider DI interface** | Allow L4 services (Environment, Faction) to modify flow calculations without Utility depending on them. Phase 7 in implementation plan. | — |
| 3 | **Variable Provider Factory for ${utility.*}** | ABML namespace for NPC infrastructure awareness in GOAP decisions. Phase 4 in implementation plan. | — |
...
Pre-implementation — no schema, no code, not listed in generated/GENERATED-SERVICE-DETAILS.md. Deep dive hardened (2026-03-11) with full tenet audit: T31 Category B deprecation lifecycle for blueprints (triple-field model, clean-deprecated sweep, no delete/undeprecate), T25 type safety (EntityType for owner/worker types, typed enums for status/pause-reason/quality-source, typed sub-models for inputs/outputs), T26 no sentinel values (nullable maxWorkers, nullable proficiencyDomain), T16 Pattern C event topics (workshop.blueprint.created not workshop-blueprint.created), T28 account deletion cleanup obligation (account.deleted handler), T7 multi-service compensation deferred to GH #626, T30 telemetry spans documented, x-lifecycle with deprecation, x-event-subscriptions, x-references for resource cleanup. 9 design considerations (1 AUDIT:NEEDS_DESIGN), 9 tracked GitHub issues.
...
| # | Enhancement | Description | Issue |
|---|-------------|-------------|-------|
| 1 | **Phase 2 - Task Lifecycle** | Implement task creation with blueprint snapshotting, two-field status model (ProductionTaskStatus + ProductionTaskPauseReason), source/destination inventory validation, and initial rate segment creation. | [#614](https://github.com/BeyondImmersion/bannou-service/issues/614) |
| 2 | **Phase 4 - Lazy Evaluation & Materialization** | Implement `ProductionCalculator` with rate segment integration, material consumption, output creation via lib-item/lib-inventory, fractional progress tracking, and auto-pause on exhaustion/capacity. | [#614](https://github.com/BeyondImmersion/bannou-service/issues/614) |
| 3 | **Materialization Compensation Strategy** | Design and implement T7-compliant compensation for multi-service partial failure during materialization (consumeMaterials succeeds, createOutputs fails). Four options under consideration. | [#626](https://github.com/BeyondImmersion/bannou-service/issues/626) |
