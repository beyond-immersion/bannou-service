...
| `GAME_SESSION_SUPPORTED_GAME_SERVICES` | string[] | `['generic']` | List of supported game service stub names. Only subscription... |
...
| Environment Variable | Type | Default | Description |
|---------------------|------|---------|-------------|
| `LOCALIZATION_CACHE_EXPIRATION_MINUTES` | int | `60` | Redis TTL in minutes for compiled export cache entries |
| `LOCALIZATION_DEFAULT_LANGUAGE` | string | `en` | Fallback language when requested language has no entry (BCP ... |
| `LOCALIZATION_DEFAULT_VALIDATION_MODE` | string | `None` | Default validation mode for new runtime categories |
| `LOCALIZATION_EXPORT_PAGE_SIZE` | int | `5000` | Entries per page when building compiled export from MySQL |
| `LOCALIZATION_LOCK_EXPIRY_SECONDS` | int | `15` | Distributed lock expiry timeout in seconds for write operati... |
| `LOCALIZATION_MAX_ENTRIES_PER_CATEGORY` | int | `10000` | Maximum translation entries per category (safety cap) |
| `LOCALIZATION_MAX_LANGUAGES` | int | `50` | Maximum supported language count across all categories |
| `LOCALIZATION_MAX_RUNTIME_CATEGORIES` | int | `100` | Maximum number of runtime-created categories (schema-defined... |
