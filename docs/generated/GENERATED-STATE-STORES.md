...
| `localization-category-store` | MySQL | Localization | Category definitions (schema-defined and runtime-created) |
| `localization-compiled-cache` | Redis | Localization | Compiled export bundles keyed by language and category (TTL-backed) |
| `localization-entry-store` | MySQL | Localization | Translation entries queryable by category, language, and key |
| `localization-lock` | Redis | Localization | Distributed locks for category and entry write operations |
...
| `realm-lock` | Redis | Realm | Distributed locks for realm merge operations |
