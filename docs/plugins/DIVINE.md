...
> **System Realm Isomorphism**: lib-divine is a thin ceremony layer over the same Genesis + Actor + Seed + Currency + Behavioral Bootstrap infrastructure shared by ALL system realm entity types. The divine economy (divinity wallets, seed bond propagation, personality-driven ABML spending) is structurally identical to the dungeon mana economy, guardian spirit progression, and any future system realm economy. What makes lib-divine a distinct plugin is the domain-specific orchestration it provides (blessing composition via Collection + Status, follower management via Relationship, attention slot mechanics) — not the underlying lifecycle, which is Genesis. Living weapons validate this composability: they use the same infrastructure with zero ceremony plugin. See [ORCHESTRATION-PATTERNS.md](../reference/ORCHESTRATION-PATTERNS.md) § The System Realm Isomorphism.
