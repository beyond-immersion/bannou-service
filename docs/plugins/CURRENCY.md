...
| lib-worldstate (`IWorldstateClient`, L2) | Both autogain modes (Lazy and Task) call `IWorldstateClient.GetElapsedGameTimeAsync(realmId, lastCheckTimestamp, currentTimestamp)` when `AutogainTimeSource` is `GameTime` (default). Wallets without `realmId` (global/account wallets) fall back to real-time. Per-definition `autogainUseGameTime` nullable boolean override allows fine-grained control (e.g., premium currencies use real-time). When Worldstate is unavailable, autogain is skipped for that wallet/cycle. |
...
| lib-actor (L2) | `${currency.*}` variable provider via `IVariableProviderFactory` — provides `${currency.balance.{CODE}}`, `${currency.locked.{CODE}}`, `${currency.has_wallet}`, `${currency.wallet_count}` for NPC economic awareness in ABML behavior expressions. Implemented with `CurrencyProviderFactory` + `CurrencyDataCache` (event-invalidated in-memory cache). |
| lib-actor (planned) | ABML economic action handlers (`economy_credit`, `economy_debit`, `economy_transfer`) calling `ICurrencyClient` for NPC economic actions. Handlers live in `bannou-service/Abml/Execution/Handlers/`, not in lib-currency. ([#428](https://github.com/beyond-immersion/bannou-service/issues/428)) |
...
| `AutogainTimeSource` | `CURRENCY_AUTOGAIN_TIME_SOURCE` | `GameTime` | Whether autogain elapsed time uses real-world time or game-world time via Worldstate (`$ref: TimeSource`). Wallets without realmId fall back to real-time regardless of this setting. |
...
2. **Entity bin wallets**: Design resolved (2026-03-20). Configurable cleanup routing — instead of burning deleted entity balances, route to destination wallets via seed-data templates (`BinWalletTemplate` registered per entity type + currency + realm). `destinationWalletId` is the compositional unlock: set to a divine entity's wallet → death god fueled by mortal wealth via Genesis ICurrencyTransactionListener → seed growth → cognitive awakening. All via configuration. Batched flushing via `EventBatcher` pattern (one credit per flush interval, not per deletion). Plugin-level master switch: `BinWalletsEnabled` (default `false`). `isBinWallet` flag on wallets, hidden from queries by default (`includeBinWallets` parameter, default `false`). See [#704](https://github.com/beyond-immersion/bannou-service/issues/704).
<!-- AUDIT:DESIGN_RESOLVED:2026-03-20:https://github.com/beyond-immersion/bannou-service/issues/704 -->
...
3. ~~**Autogain uses real-time, should use game-time**~~ ([#545](https://github.com/beyond-immersion/bannou-service/issues/545), **IMPLEMENTED** 2026-03-20): `AutogainTimeSource` config property (`$ref: TimeSource`, default: `GameTime`) controls whether autogain uses real-time or game-time. Wallets with `realmId` call `IWorldstateClient.GetElapsedGameTimeAsync`; wallets without `realmId` fall back to real-time. Per-definition `autogainUseGameTime` nullable boolean override for fine-grained control. Both `Lazy` and `Task` modes use the same logic.
...
*(No pending design items — all design questions resolved.)*
...
- [#473](https://github.com/beyond-immersion/bannou-service/issues/473) - Item linkage: Add `IItemClient` dependency for creation-time template existence validation when `LinkedToItem=true`. `ItemLinkageMode` enum already removed from schema.
- [#478](https://github.com/beyond-immersion/bannou-service/issues/478) - Universal value anchoring + location-scoped modifiers: Three-layer architecture following CalendarEvent (#538) pattern. `UniversalValue` coexists with `ExchangeRateToBase`. `ExchangeRateModifier` entity with scope/expiry/source/stacking. ABML action `economy_add_rate_modifier`.
- ~~[#545](https://github.com/beyond-immersion/bannou-service/issues/545)~~ - Game-time autogain: **IMPLEMENTED** (2026-03-20). `AutogainTimeSource` config (`$ref: TimeSource`, default `GameTime`). Wallets with `realmId` call `IWorldstateClient.GetElapsedGameTimeAsync`; wallets without `realmId` fall back to real-time. Per-definition `autogainUseGameTime` override. Both Lazy and Task modes use same logic.
- [#556](https://github.com/beyond-immersion/bannou-service/issues/556) - Non-account entity deletion wallet cleanup: `POST /currency/cleanup-by-owner` endpoint via lib-resource `x-references` CASCADE. Shares cleanup logic with existing account deletion handler. Burns balances by default; routes to bin wallets when #704 is enabled. x-references for `target: character` first; faction/organization added when implemented.
- [#704](https://github.com/beyond-immersion/bannou-service/issues/704) - Entity bin wallets: configurable cleanup routing via seed-data templates. Destination wallets receive deleted entity balances (batched via EventBatcher). `BinWalletsEnabled` config (default false). Composes with Genesis for emergent god-awakening from mortal wealth.
- [#651](https://github.com/beyond-immersion/bannou-service/issues/651) - Batch lifecycle events: Two-track design. (1) CurrencyBalance x-lifecycle → `batch: true` (auto-generated batch event types). (2) Custom economic events (credited/debited/transferred) → `EventBatcher<T>` with custom batch schemas. Self-subscription handlers update to iterate batch entries. Client events + low-frequency events unchanged. ICurrencyTransactionListener stays individual-call (Genesis in-memory batching handles volume; DI listener overhead is microseconds). Existing batch API endpoints unchanged (partial success semantics preserved).

### Active

*(No active work items — all moved to Design Resolved.)*

### Completed

- **2026-03-20**: Batch lifecycle events design resolved (#651). Two-track: CurrencyBalance x-lifecycle gets `batch: true` (generated batch types); custom economic events (credited/debited/transferred) get `EventBatcher<T>` with custom batch schemas + `EventBatcherWorker`. Self-subscription cache invalidation updates to iterate batch entries. Client events and low-frequency events (wallet/hold/exchange rate) stay individual. `ICurrencyTransactionListener` stays individual-call — Genesis's in-memory ConcurrentDictionary handles volume at microsecond cost; adding batch overloads would be a breaking interface change for a non-existent bottleneck. Existing batch API endpoints (`BatchCredit`/`BatchDebit`) unchanged — aggregate-transaction semantics (single lock for N items) deferred as a future optimization with its own cap-enforcement complexity.
- **2026-03-20**: Entity bin wallets extension designed (#704). Configurable routing of deleted entity balances to destination wallets via seed-data `BinWalletTemplate`s. Composes emergently with Genesis: destination = divine wallet → ICurrencyTransactionListener → seed growth → god awakening. Batched via EventBatcher (one credit per flush, not per deletion). Plugin-level `BinWalletsEnabled` (default false). `isBinWallet` flag + `includeBinWallets` query parameter.
- **2026-03-20**: Non-account entity deletion wallet cleanup design resolved (#556). `POST /currency/cleanup-by-owner` endpoint via lib-resource `x-references` CASCADE. Reuses cleanup logic from existing `HandleAccountDeletedAsync` (extract shared `CleanupWalletsForOwnerAsync`). Burns balances (currency sink, measurable via supply counters). Releases active holds. Force-closes frozen wallets. x-references for `target: character` first; faction/organization added when those L4 services are implemented. Endpoint generic for any ownerType.
