...
- [#545](https://github.com/beyond-immersion/bannou-service/issues/545) - Game-time autogain: Add `AutogainTimeSource` config (`$ref: TimeSource`, default `GameTime`). Wallets with `realmId` call `IWorldstateClient.GetElapsedGameTimeAsync`; wallets without `realmId` fall back to real-time. Both Lazy and Task modes use same logic. Worldstate API is implemented — no blocker.
