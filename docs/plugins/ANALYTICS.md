...
| `SummaryResetBatchSize` | `ANALYTICS_SUMMARY_RESET_BATCH_SIZE` | 100 | Records to delete per iteration during summary reset |
| `SummaryResetLockExpirySeconds` | `ANALYTICS_SUMMARY_RESET_LOCK_EXPIRY_SECONDS` | 60 | Distributed lock expiry for summary reset operations |
| `RatingDecayInactivityDays` | `ANALYTICS_RATING_DECAY_INACTIVITY_DAYS` | 30 | Days of inactivity before Glicko-2 RD decay begins |
| `RatingDecayIntervalSeconds` | `ANALYTICS_RATING_DECAY_INTERVAL_SECONDS` | 3600 | Interval between decay worker cycles (1 hour default) |
| `RatingDecayStartupDelaySeconds` | `ANALYTICS_RATING_DECAY_STARTUP_DELAY_SECONDS` | 60 | Startup delay before first decay cycle |
| `RatingDecayBatchSize` | `ANALYTICS_RATING_DECAY_BATCH_SIZE` | 500 | Maximum inactive ratings to process per decay cycle |
| `RatingDecayLockExpirySeconds` | `ANALYTICS_RATING_DECAY_LOCK_EXPIRY_SECONDS` | 300 | Distributed lock expiry for decay operations |
...
### ~~Rating Period Decay~~
<!-- AUDIT:RESOLVED:2026-03-26:https://github.com/beyond-immersion/bannou-service/issues/249 -->

~~The Glicko-2 algorithm includes a concept of "rating period decay" where a player's rating deviation increases over time when they don't play.~~ **IMPLEMENTED** (2026-03-26) — `AnalyticsRatingDecayWorker` background service with Redis sorted set index (`rating-decay-tracker`) scored by `LastMatchAt`. See Work Tracking § Recently Implemented.
...
- ~~**Auth audit event consumption (Phase 1)**~~: serviceType/serviceId refactoring and auth event ingestion into existing pipeline. **IMPLEMENTED** (2026-03-26) — [#142](https://github.com/beyond-immersion/bannou-service/issues/142) closed. See Work Tracking § Recently Implemented.
<!-- AUDIT:RESOLVED:2026-03-26:https://github.com/beyond-immersion/bannou-service/issues/142 -->
- **Auth IP geolocation enrichment (Phase 2)**: `IGeolocationService` singleton helper with db-ip MMDB + MaxMind.GeoIP2 reader for IP-to-location enrichment of auth events. Per-IP entity tracking with deterministic UUID v5 GUIDs. Depends on Phase 1 (implemented). See [#639](https://github.com/beyond-immersion/bannou-service/issues/639).
<!-- AUDIT:DESIGN_RESOLVED:2026-03-20:https://github.com/beyond-immersion/bannou-service/issues/639 -->
- **Auth anomaly detection (Phase 3)**: Cross-account IP correlation with time-windowed distinct counting and `SecurityAlertClientEvent` admin alerting. Depends on Phase 2. New issue to be created.
- ~~**Event replay**~~: Ability to reprocess buffered events after a bug fix. **REJECTED** (2026-03-20) — [#634](https://github.com/beyond-immersion/bannou-service/issues/634) closed as wrong-layer. Event archival/replay is an observability stack concern (Loki/OTEL/Grafana, see #183/#185), not Analytics application code. Corrupted summary recovery is handled by the admin reset endpoint ([#707](https://github.com/beyond-immersion/bannou-service/issues/707), implemented 2026-03-26). Source event data is durable in owning services (character-history, realm-history, game-session MySQL).
<!-- AUDIT:REJECTED:2026-03-20:https://github.com/beyond-immersion/bannou-service/issues/634 -->
- ~~**Time-series queries**~~: Adding time-bucketed aggregations for trend analysis. **REJECTED** (2026-03-20) — [#635](https://github.com/beyond-immersion/bannou-service/issues/635) closed as wrong-layer. Time-series storage and querying is the observability stack's job (Prometheus/Grafana). The actual gap was metric emission — resolved by [#708](https://github.com/beyond-immersion/bannou-service/issues/708) (implemented 2026-03-26, see Work Tracking § Recently Implemented).
<!-- AUDIT:REJECTED:2026-03-20:https://github.com/beyond-immersion/bannou-service/issues/635 -->
...
5. **`string.Empty` default for internal POCO string fields**: `BufferedAnalyticsEvent.EventType`, `BufferedAnalyticsEvent.ServiceId`, `GameSessionMappingData.GameType`, `SkillRatingData.RatingType`, `SkillRatingData.ServiceId`, `EntitySummaryData.ServiceId`, and `ControllerHistoryData.ServiceId` use `= string.Empty` defaults. These are standard NRT compliance — every construction site sets these fields explicitly, and the defaults only serve as deserialization safety for non-nullable string properties.
...
- [#142](https://github.com/beyond-immersion/bannou-service/issues/142) - **Auth audit event consumption (Phase 1)** + serviceType/serviceId prerequisite refactoring. Implemented 2026-03-26. Prerequisite: `gameServiceId: Guid` → `serviceType: AnalyticsServiceType` + `serviceId: string` across all schemas, models, events, and key builders. `AnalyticsServiceType` enum: `Game` (GUID, resolved via IGameServiceClient), `System` (logical service name, no resolution). Entity key pattern: `analytics-entity:{serviceType}:{serviceId}:{entityType}:{entityId}`. Consumers (Achievement, Leaderboard) filter `serviceType == Game`. 10 auth event subscriptions buffered with `serviceType: System, serviceId: "auth"`, per-account entity summaries. Failed logins with null `accountId` dropped.

- [#707](https://github.com/beyond-immersion/bannou-service/issues/707) - **Admin summary reset endpoint**. Implemented 2026-03-26. `POST /analytics/summary/reset` (admin-only) deletes entity summaries for a `serviceType`/`serviceId` scope with optional `entityType`/`entityId` filters. Dry-run mode (default: true). Distributed lock prevents concurrent resets. Summaries rebuild naturally as new events arrive. Replaces #634 (event replay, rejected as wrong-layer).

- [#708](https://github.com/beyond-immersion/bannou-service/issues/708) - **RecordCounter metric emission during buffer flush**. Implemented 2026-03-26. `analytics.score.processed` (counter, tagged by service_type/service_id/entity_type/score_type) and `analytics.events.processed` (counter, tagged by service_type/service_id) emitted during `FlushBufferedEventsBatchAsync`. Enables Prometheus/Grafana time-series analysis without custom storage.

- [#249](https://github.com/beyond-immersion/bannou-service/issues/249) - **Glicko-2 rating period decay**. Implemented 2026-03-26. `AnalyticsRatingDecayWorker` background service finds inactive ratings via Redis sorted set index (`rating-decay-tracker` scored by `LastMatchAt` ms), applies the existing zero-games Glicko-2 formula to increase RD by volatility, and publishes `analytics.rating.updated` with `MatchId = null`. Self-limiting: removes entries when RD reaches `Glicko2DefaultDeviation`. `matchId` on `AnalyticsRatingUpdatedEvent` made nullable (null = decay, non-null = match). Sorted set maintained by `UpdateSkillRatingAsync` (add/update on match), decay worker (remove at max RD), and `HandleAccountDeletedAsync` (cleanup). Config: `RatingDecayInactivityDays` (30), `RatingDecayIntervalSeconds` (3600), `RatingDecayStartupDelaySeconds` (60), `RatingDecayBatchSize` (500), `RatingDecayLockExpirySeconds` (300).
...
- [#639](https://github.com/beyond-immersion/bannou-service/issues/639) - Auth IP geolocation enrichment (Phase 2). `IGeolocationService` singleton helper with `[BannouHelperService]`. MaxMind.GeoIP2 reader (Apache 2.0) + db-ip.com "IP to City Lite" MMDB (CC BY 4.0, no account). Config: `GeoIpDatabasePath` (nullable, null = disabled). Enriches auth events at ingestion with country/city. Per-IP entity tracking: deterministic GUID from IP string via namespace UUID v5, `EntityType.Other`. Graceful degradation when MMDB absent. Depends on Phase 1 (implemented).
