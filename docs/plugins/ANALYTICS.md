...
- [#708](https://github.com/beyond-immersion/bannou-service/issues/708) - Add `RecordCounter` metric emission during buffer flush. ~10 lines: `analytics.score.processed` (counter, tagged by score_type/entity_type/service) and `analytics.events.processed` (counter, tagged by service). Enables Prometheus/Grafana time-series analysis (kills per hour, gold per day, faucet/sink trends, velocity derivation) without custom time-series storage. Replaces #635 (time-series queries, rejected as wrong-layer).
