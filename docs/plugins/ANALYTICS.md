...
- ~~**Auth audit event consumption (Phase 1)**: serviceType/serviceId refactoring and auth event ingestion into existing pipeline.~~ **IMPLEMENTED** (2026-03-26) — #142 closed. Prerequisite refactoring (`gameServiceId: Guid` → `serviceType: AnalyticsServiceType` + `serviceId: string`) and 10 auth event subscriptions implemented. See Work Tracking.
...
- ~~**Time-series queries**: Adding time-bucketed aggregations for trend analysis~~ **REJECTED** (2026-03-20) — #635 closed as wrong-layer. Time-series storage and querying is the observability stack's job (Prometheus/Grafana), not a custom MySQL-backed store inside Analytics. The actual gap is metric emission: Analytics emits zero `RecordCounter` calls during flush. Adding ~10 lines of counter emission with score_type/entity_type/service tags gives Prometheus everything it needs. See [#708](https://github.com/beyond-immersion/bannou-service/issues/708). Same pattern as #634 (event archival → Loki, not custom archive).
