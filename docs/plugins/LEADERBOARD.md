...
| `leaderboard.definition.deleted` | `LeaderboardDefinitionDeletedEvent` | Unused Category B infrastructure — definitions are never deleted. Generated by `x-lifecycle` for future safe deletion pattern. |
...
- **ListLeaderboardDefinitions** (`/leaderboard/definition/list`): Loads all definition IDs from gameService index set. Supports `includeDeprecated` (default false, per IMPLEMENTATION TENETS). Filters `IncludeArchived` returns `NotImplemented`. Loads each definition individually with entry count. Sorted alphabetically by leaderboard ID.
...
- **DeprecateLeaderboardDefinition** (`/leaderboard/definition/deprecate`): Marks definition as deprecated (idempotent — returns OK if already deprecated). Publishes `leaderboard.definition.updated` event with `changedFields` containing deprecation fields. No delete endpoint — leaderboard definitions are Category B entities (instances/scores persist independently).

### Scores (2 endpoints)

- **SubmitScore** (`/leaderboard/score/submit`): Rejects with BadRequest if definition is deprecated (Category B instance creation guard). Validates entity type against definition's allowed types. Gets previous score and rank. Calculates final score via `CalculateFinalScore` (Replace/Increment/Max/Min). Adds to sorted set. Publishes `entry.added` for first-time entities or `rank.changed` if rank shifted. Returns previous/current score and rank with rank change delta.
...
<!-- AUDIT:NEEDS_DESIGN:2026-03-06:event publishing for batch requires per-entry rank lookups which negates batch performance; consider post-batch summary event -->
...
<!-- AUDIT:NEEDS_DESIGN:2026-03-06:compliance for Increment/Max/Min modes requires Lua scripts or distributed locks for atomicity -->
...
<!-- AUDIT:NEEDS_DESIGN:2026-03-06:dead config; remove unused store or implement season archival with MySQL persistence -->
