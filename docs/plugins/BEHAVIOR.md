...
1. **Vocalization channel validation gap**: `SemanticAnalyzer.ValidateEmitIntent` checks against `["action", "locomotion", "attention", "stance"]` but the opcode enum defines a 5th channel `Vocalization = 4`. Using `emit_intent` with channel "vocalization" will produce a semantic warning despite being a valid runtime channel.

2. **ValidateAbml always returns empty SemanticWarnings**: At line 530 of BehaviorService.cs, `SemanticWarnings = new List<string>()` is hardcoded. The compilation result from `_compiler.CompileYaml()` would produce warnings (unreachable code, unused flows, unknown intent channels) but they are never mapped to the validation response. The `/validate` endpoint can report errors but never warnings.

3. **ValidateAbml runs full compilation pipeline unnecessarily**: At line 514 of BehaviorService.cs, `/validate` calls `_compiler.CompileYaml(body.AbmlContent, options)` which executes the entire pipeline including flow compilation and bytecode emission. The generated bytecode is discarded. This wastes CPU for validation-only requests - should use a validation-specific compiler path that stops after semantic analysis.
...
16. **BehaviorModelCache.GetInterpreter race condition on cold cache**: At lines 142-158 of BehaviorModelCache.cs, two concurrent threads calling `GetInterpreter` for the same character/type/variant can both miss the cache check (line 144), both create separate `BehaviorModelInterpreter` instances (line 155), and both write to the cache via direct assignment. The last writer wins and the earlier caller's interpreter is evicted from the cache while potentially still in use. Should use `GetOrAdd` on the ConcurrentDictionary.

17. **CompilationContext local variable index overflows silently at 256**: At line 134 of CompilationContext.cs, `_nextLocalIndex` is a `byte` that wraps to 0 when 256 locals are allocated. No bounds check prevents overflow. The `in` operator's `__in_lhs_{label}` synthetic locals (line 274 of StackExpressionCompiler.cs) mean deeply nested expressions with many `in` operators could exhaust the local space and silently corrupt the variable table.
