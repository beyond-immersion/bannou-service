...
1. **~~Vocalization channel validation gap~~** *(FIXED)*: Added "vocalization" to the valid channels list in `SemanticAnalyzer.ValidateEmitIntent`.

2. **~~ValidateAbml always returns empty SemanticWarnings~~** *(FIXED)*: Added `Warnings` property to `CompilationResult`, captured from `SemanticAnalysisResult` during compilation, and mapped to the validate response.

3. **ValidateAbml runs full compilation pipeline unnecessarily**: At line 514 of BehaviorService.cs, `/validate` calls `_compiler.CompileYaml(body.AbmlContent, options)` which executes the entire pipeline including flow compilation and bytecode emission. The generated bytecode is discarded. This wastes CPU for validation-only requests - should use a validation-specific compiler path that stops after semantic analysis. *(Reclassified as Design Consideration - the endpoint is correct, just suboptimal.)*
