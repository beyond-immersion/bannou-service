...
- **ValidateAbml** (`/validate`): Validates ABML YAML without producing bytecode. Runs the same parser and semantic analyzer as `/compile` but stops after the analysis phase. Returns validation result with `isValid` flag, error list (undefined flows, empty conditionals, invalid continuation points, type mismatches), and warning list (unreachable code after goto/return, unused flows, empty flows, unknown intent channels, zero-repeat loops). Does not modify state or publish events.
...
1. **Memory store loads all memories for relevance scoring**: `FindRelevantAsync` calls `GetAllAsync` to load the entity's full memory set, then scores each one against all perceptions. With the default limit of 100 memories and 10 perceptions, this is 1000 relevance calculations per query. Scaling to larger memory sets would require pre-filtering or indexing.
