...
|-------|-----------|---------|
| `character-encounter.encounter-type.created` | `EncounterTypeCreatedEvent` | Encounter type created (lifecycle) |
| `character-encounter.encounter-type.updated` | `EncounterTypeUpdatedEvent` | Encounter type updated or deprecated via changedFields (lifecycle) |
| `character-encounter.encounter-type.deleted` | `EncounterTypeDeletedEvent` | Unused Category B infrastructure — encounter types are never deleted. Generated by `x-lifecycle` for future safe deletion pattern. |
