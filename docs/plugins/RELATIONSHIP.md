...
<!-- AUDIT:NEEDS_DESIGN:2026-03-15:https://github.com/beyond-immersion/bannou-service/issues/661 -->
...
- ~~[#544](https://github.com/beyond-immersion/bannou-service/issues/544)~~: Game-time auto-population for `startedAt`/`endedAt` fields — **resolved (2026-03-19): not applicable**. Relationships are cross-realm entities (entity A in realm 1, entity B in realm 2) with no single clock. Game-time auto-population does not apply. `startedAt`/`endedAt` remain caller-provided DateTimeOffset values. Character-Encounter (the other service in #544) does get game-time migration
...
- [#661](https://github.com/beyond-immersion/bannou-service/issues/661): Hard-delete ended relationships (T28 deletion finality) — soft-delete pattern violates Foundation Tenets; requires design decision on historical data preservation mechanism (Bug #1)
