...
1. **Intent-based chat classification with persona mapping**: Transform raw platform chat into structured intent streams with anonymous character personas, directly consumable by lib-showtime's ABML-driven audience system. Two-layer hybrid classifier: Layer 1 is emote/keyword lookup from Resource seed data (sub-microsecond, handles ~60% of Twitch chat, viable as standalone v1); Layer 2 is ML.NET SDCA multiclass classifier (MIT, ~3.5 MB, trained from seed phrases at startup, sub-millisecond inference, optional upgrade). Chatters are anonymized into a seed-data-defined character roster via behavioral fingerprinting (ALL CAPS → loud character, high-frequency → chatty character) with LRU reuse. **Supersedes the original cross-session returner detection** (privacy-complex, less valuable than structured intents). Returner detection remains available as a sub-feature via #569 if needed. See [GitHub Issue #673](https://github.com/beyond-immersion/bannou-service/issues/673).
