...
| `createdBy` | -- (Informational) | Plain string (required) | Caller-provided descriptive label recording who created the asset/bundle. Required everywhere (matches bundle-format SDK manifest contract). Not used for access control, not polymorphic — purely informational for filtering and audit. |

**Notes**:
- Asset service has no `EntityType` enum fields (Category A). `createdBy` is a plain required string — informational, not access control.
...
| `AdditionalProcessableContentTypes` | `ASSET_ADDITIONAL_PROCESSABLE_CONTENT_TYPES` | *nullable* | Additional processable MIME types (string array) |
| `AdditionalExtensionMappings` | `ASSET_ADDITIONAL_EXTENSION_MAPPINGS` | *nullable* | Additional `.ext=type` mappings (string array) |
| `AdditionalForbiddenContentTypes` | `ASSET_ADDITIONAL_FORBIDDEN_CONTENT_TYPES` | *nullable* | Additional forbidden MIME types (string array) |
...
2. ~~**`owner` event field violates polymorphic typing and account identity boundary**~~: **FIXED** (2026-03-15) - Replaced polymorphic `ownerType` enum + `ownerId` with a single required `createdBy` string field across all request models, response models, events, and internal storage models. Removed the `AssetOwnerType` enum entirely. The field is explicitly informational (not used for access control) and required everywhere to match the bundle-format SDK's `BundleManifest.CreatedBy` contract. Callers provide a descriptive string of their choice. No T14/T32 violations remain since there is no polymorphic typing and no accountId anywhere.

3. ~~**Orchestrator hard dependency violates L3 graceful degradation requirement**~~: **FIXED** (2026-03-15) - Replaced constructor-injected `IOrchestratorClient` with `IServiceProvider` and soft resolution via `GetService<IOrchestratorClient>()` at call sites. When Orchestrator is absent, pool scaling and processor acquisition are skipped with log messages. `AssetProcessingWorker` also uses `GetService` instead of `GetRequiredService`. Asset no longer crashes at startup when Orchestrator is disabled.

4. ~~**Comma-delimited string configuration properties**~~: **FIXED** (2026-03-15) - Changed `AdditionalProcessableContentTypes`, `AdditionalExtensionMappings`, and `AdditionalForbiddenContentTypes` from `type: string` (comma-delimited) to `type: array, items: { type: string }` in the configuration schema. Updated `ContentTypeRegistry.ApplyConfigurationExtensions` to iterate arrays directly instead of `.Split(',')`. Extension mappings still use `=` within each array element (e.g., `".ktx=image/ktx"`) since the key-value pair is a single atomic value, not structured data.

5. ~~**MinIO connectivity check uses try/finally instead of `using`**~~: **FIXED** (2026-03-15) - Replaced try/finally with `using var` declaration per IMPLEMENTATION TENETS (T24). The fluent builder pattern (`new MinioClient().WithEndpoint(...).WithCredentials(...)`) works correctly with `using` since `Build()` returns `this`.
...
- **Owner field simplified to createdBy (Bug #2)**: Replaced polymorphic `ownerType` enum + `ownerId` with single required `createdBy` string. Removed `AssetOwnerType` enum. Field is informational (not access control), required to match bundle-format SDK manifest contract. (2026-03-15)
- **Orchestrator soft dependency (Bug #3)**: Replaced constructor-injected `IOrchestratorClient` with `IServiceProvider` soft resolution. Asset now degrades gracefully when Orchestrator is disabled. (2026-03-15)
- **Comma-delimited config to arrays (Bug #4)**: Changed three `string` config properties to `string[]` arrays per IMPLEMENTATION TENETS (T21). Updated `ContentTypeRegistry` to iterate arrays directly. (2026-03-15)
- **MinIO try/finally to using (Bug #5)**: Replaced try/finally disposal with `using var` in `WaitForMinioConnectivityAsync` per IMPLEMENTATION TENETS (T24). (2026-03-15)
