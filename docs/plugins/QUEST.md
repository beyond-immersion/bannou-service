...
4. **Wallet-debit-based objective tracking**: The "Bag of Kills" pattern from [DIVINITY-GENERATION-ARCHITECTURE.md](../planning/DIVINITY-GENERATION-ARCHITECTURE.md) proposes Currency wallets as universal accumulators where multiple systems consume different dimensions of the same underlying events. For Quest: the game engine credits a character's domain wallet (e.g., `combat_experience`), Quest debits from the wallet balance to complete kill/collect objectives, Genesis maps the same credits to permanent seed growth, and Divine receives growth via seed bond propagation — all independently, no contention. This would replace `ObjectiveProgressModel` Redis tracking (with its TTL durability concern — see DC #1) with durable MySQL-backed wallet balances for at least the KILL, COLLECT, and CRAFT objective types. The CUSTOM objective type with external progress reporting would remain for objectives that don't map to a countable domain. Genesis's batched growth flush ensures wallet credits don't create back-pressure on the Seed pipeline even at 100K+ NPC scale. See also [GENESIS.md § Core Mechanics](GENESIS.md) for the batched flush architecture.

5. **Client events for real-time objective tracking** ([#496](https://github.com/beyond-immersion/bannou-service/issues/496)): Push `QuestObjectiveProgressed` and `QuestStatusChanged` (consolidated lifecycle event with status discriminator for accepted/completed/failed/abandoned) client events via `IClientEventPublisher` using the Entity Session Registry (#426, now implemented). Sessions resolved via `character → session` bindings for all questor characters. Published from Quest's event handlers that process Contract state transitions. **Unblocked** — infrastructure dependency (#426) is closed.
...
| `prerequisiteType` | C (System State) | `PrerequisiteType` enum | Prerequisite types. Built-in (L2): `QUEST_COMPLETED`, `ITEM_OWNED`, `CURRENCY_AMOUNT` use direct service client calls. All other types route through `IPrerequisiteProviderFactory` DI pattern for L4/L5 extensibility. **Note**: `CHARACTER_LEVEL` and `REPUTATION` are game concepts scheduled for removal from the enum — games define their own prerequisite types via dynamic providers. See GH#719. |
...
1. **Objective progress durability** ([#562](https://github.com/beyond-immersion/bannou-service/issues/562)): Redis-only progress storage with 5-minute TTL (`ProgressCacheTtlSeconds`) causes silent data loss for long-running quests (daily, weekly, exploration). Contract milestones are binary (complete/not complete) and cannot store partial progress, so progress counts and entity deduplication data (`TrackedEntityIds`) have no durable backing. If no progress is reported for longer than the TTL, partial progress is silently lost. This is a first-class scaling concern — the current default TTL is appropriate for rapid combat quests but inadequate for any quest type with intermittent progress updates. **Proposed alternative**: The "Bag of Kills" pattern from [DIVINITY-GENERATION-ARCHITECTURE.md](../planning/DIVINITY-GENERATION-ARCHITECTURE.md) proposes using Currency wallet debits as the objective completion mechanism — the game engine credits a character's domain wallet (e.g., "combat_experience"), and Quest debits from the wallet balance to complete objectives. Wallet balances are MySQL-backed (durable), eliminate the TTL concern entirely, and the same wallet simultaneously feeds Genesis growth (permanent) and Divine bond propagation (independent) with zero contention between consumers. This would shift KILL/COLLECT objectives from internal Redis tracking to wallet-debit validation, resolving the durability problem architecturally rather than by extending TTLs.
<!-- AUDIT:NEEDS_DESIGN:2026-03-04:https://github.com/beyond-immersion/bannou-service/issues/562 -->

2. **Prerequisite architecture — `CharacterLevel` and `Reputation` must be removed from enum**: The two-tier prerequisite system (built-in L2 types + dynamic `IPrerequisiteProviderFactory`) is architecturally correct (#320). However, `CharacterLevel` and `Reputation` are **game implementation concepts baked into a Bannou enum** — the same anti-pattern as building dedicated combat/skill/magic plugins (see FAQ: WHY-ARE-THERE-NO-SKILL-MAGIC-OR-COMBAT-PLUGINS). Every game defines "level" and "reputation" differently. These enum values should be removed from `PrerequisiteType`, along with their dedicated schema fields (`minLevel`, `factionCode`, `minReputation`). Games define their own prerequisite types via the dynamic provider path — an L5 extension registers an `IPrerequisiteProviderFactory` for whatever prerequisite types their game needs (querying Seed capabilities, Currency balances, or any other source). The `PrerequisiteType` enum should either become an opaque string (Category B, game-configurable) or retain only the built-in L2 types (`QuestCompleted`, `ItemOwned`, `CurrencyAmount`) with all else routing through the dynamic path. The `IPrerequisiteProviderFactory` interface XML docs also incorrectly list `character_level` as built-in — needs correction. See GH#719 for the full schema cleanup.
<!-- AUDIT:NEEDS_DESIGN:2026-03-25:https://github.com/beyond-immersion/bannou-service/issues/719 -->
