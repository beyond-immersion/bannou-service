...
2. ~~**Prerequisite architecture**~~: **FIXED** (2026-02-07, [#320](https://github.com/beyond-immersion/bannou-service/issues/320)) - Quest uses a two-tier prerequisite system: built-in (L2) types (`quest_completed`, `currency`, `item`) via direct service client calls, and dynamic types via `IPrerequisiteProviderFactory` DI collection for L4 extensibility. See `docs/reference/SERVICE-HIERARCHY.md` for the full pattern. **Open concern — `CHARACTER_LEVEL` and `REPUTATION` are silent no-ops**: Both are enum values in `PrerequisiteType` that route through `CheckDynamicPrerequisiteAsync` to `IPrerequisiteProviderFactory`. However, **no provider is registered for either type** — the only registered provider is `AchievementPrerequisiteProviderFactory` in lib-achievement. When no provider is found, Quest silently passes the check (graceful degradation). This means any quest with a `CHARACTER_LEVEL` or `REPUTATION` prerequisite currently has NO gate — any character can accept it. **Resolution**: lib-status (L4) should implement `StatusPrerequisiteProviderFactory` with `ProviderName = "character_level"` (and potentially `"reputation"`). Status is THE unified effects query layer — it already aggregates seed-derived capabilities and item-based effects via `GetEffects`. A "character level" check is "does this character have capability X at depth Y?" — exactly what Status answers. Status is L4, Quest is L2 — the `IPrerequisiteProviderFactory` DI inversion pattern is architecturally correct for this. Reputation could similarly be a seed-derived effect (faction standing) or a Status effect exposed through the unified query. The `IPrerequisiteProviderFactory` interface XML docs in `bannou-service/Providers/IPrerequisiteProviderFactory.cs` incorrectly list `character_level` as built-in (L2) — the code correctly routes it as dynamic. The XML docs should be corrected.
