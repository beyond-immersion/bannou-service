...
2. **Anonymous compressed event**: `character.compressed` is published as an anonymous object instead of a typed event model, violating FOUNDATION TENETS.
3. **Dead `CharacterRetentionDays` config**: Defined but never referenced in service code.
