...
1. **[IMPLEMENTATION] Missing ApiException catch blocks in all service methods (T7 Error Handling)**: Every public method in `AchievementService.cs` catches only `Exception` but never catches `ApiException` specifically. The T7 standard requires distinguishing between expected API errors (log as Warning, propagate status) and unexpected exceptions (log as Error, emit error event). This affects all 9 endpoint methods (`CreateAchievementDefinitionAsync`, `GetAchievementDefinitionAsync`, `ListAchievementDefinitionsAsync`, `UpdateAchievementDefinitionAsync`, `DeleteAchievementDefinitionAsync`, `GetAchievementProgressAsync`, `UpdateAchievementProgressAsync`, `UnlockAchievementAsync`, `ListUnlockedAchievementsAsync`, `SyncPlatformAchievementsAsync`, `GetPlatformSyncStatusAsync`). File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`.

2. **[IMPLEMENTATION] Hardcoded lock timeout of 30 seconds (T21 Configuration-First)**: The `_lockProvider.LockAsync()` calls at lines 550 and 723 of `AchievementService.cs` use a hardcoded `30` second expiry. Per IMPLEMENTATION TENETS, any tunable value (limits, timeouts, thresholds) MUST be a configuration property. This should be a configuration property like `LockExpirySeconds` in the achievement configuration schema. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 550 and 723.

3. **[IMPLEMENTATION] Hardcoded maxAttempts = 3 in IncrementEarnedCountAsync (T21 Configuration-First)**: The `const int maxAttempts = 3` at line 1183 of `AchievementService.cs` is a hardcoded tunable for the ETag conflict retry count. Per IMPLEMENTATION TENETS, this should be a configuration property (e.g., `EarnedCountRetryAttempts`). File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, line 1183.

4. **[IMPLEMENTATION] Hardcoded epsilon tolerance 0.000001 (T21 Configuration-First)**: The floating-point comparison tolerance `0.000001` is hardcoded in `TryConvertDeltaToIncrement` (line 453), `IsCloseTo` (line 463), and `TryGetMetadataLong` (line 544) in `AchievementServiceEvents.cs`. While borderline, these are numeric comparison thresholds that could be considered tunables. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementServiceEvents.cs`, lines 453, 463, 544.

5. **[IMPLEMENTATION] Unused state store `achievement-unlock` defined in state-stores.yaml (T21 Configuration-First)**: The `achievement-unlock` store is defined in `schemas/state-stores.yaml` and generated in `StateStoreDefinitions.AchievementUnlock` but is never referenced anywhere in the achievement service code. Per IMPLEMENTATION TENETS, defined state stores MUST be used or removed from the schema. File: `schemas/state-stores.yaml` (achievement-unlock entry).

6. **[IMPLEMENTATION] Dead configuration properties: XboxClientId, XboxClientSecret, PlayStationClientId, PlayStationClientSecret (T21 Configuration-First)**: These four configuration properties are defined in `schemas/achievement-configuration.yaml` but are never referenced by any service code. The Xbox and PlayStation stub implementations (`XboxAchievementSync.cs`, `PlayStationAchievementSync.cs`) hardcode `IsConfigured => false` and never read these configuration values. Per IMPLEMENTATION TENETS, every defined config property MUST be referenced in service code. File: `schemas/achievement-configuration.yaml` and `plugins/lib-achievement/Generated/AchievementServiceConfiguration.cs`.

7. **[IMPLEMENTATION] PlatformIds dictionary uses string keys instead of Platform enum (T25 Internal Model Type Safety)**: The internal `AchievementDefinitionData` model uses `Dictionary<string, string>? PlatformIds` (line 1705 of `AchievementService.cs`) where the keys represent platform names as lowercase strings. The service performs `platform.ToString().ToLowerInvariant()` lookups (lines 995, 1311). Per IMPLEMENTATION TENETS, internal POCOs should use the strongest available C# type -- this should be `Dictionary<Platform, string>?` with enum keys. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, line 1705.

8. **[IMPLEMENTATION] Metadata property typed as `object?` in internal POCO (T25 Internal Model Type Safety)**: The `AchievementDefinitionData.Metadata` property (line 1713) is typed as `object?` which loses all type safety. While the metadata is by design a flexible dictionary, it could at minimum be `Dictionary<string, object>?` or `IReadOnlyDictionary<string, object>?` rather than a raw `object?`. The event handlers already convert it via `MetadataHelper.ConvertToReadOnlyDictionary(definition.Metadata)` on every access, indicating the stored type is not optimal. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, line 1713.

9. **[IMPLEMENTATION] Direct JsonDocument.Parse usage in SteamAchievementSync instead of BannouJson (T20 JSON Serialization)**: `SteamAchievementSync.cs` uses `JsonDocument.Parse(responseBody)` at lines 401 and 471 to parse Steam API responses. Per IMPLEMENTATION TENETS, all JSON deserialization MUST use `BannouJson`. While parsing external API responses is a boundary, the tenet states "Direct use of `JsonSerializer` is forbidden except in unit tests" with no exception for external APIs. The `using System.Text.Json` import at line 5 supports this usage. File: `/home/lysander/repos/bannou/plugins/lib-achievement/Sync/SteamAchievementSync.cs`, lines 401 and 471.

10. **[IMPLEMENTATION] Unused `using System.Text.Json` import in AchievementServiceEvents.cs (T20 JSON Serialization)**: Line 7 of `AchievementServiceEvents.cs` imports `System.Text.Json` but the file only uses `JsonElement` in the `TryGetMetadataDouble` and `TryGetMetadataLong` helper methods. The `JsonElement` type is in `System.Text.Json` namespace but the usage pattern (checking `ValueKind`, calling `GetString()`, `TryGetInt64()`, `TryGetDouble()`) is for reading already-deserialized metadata rather than performing new deserialization. This is a borderline case -- the import itself indicates potential T20 proximity, though the actual usage is reading pre-parsed JsonElement values from the metadata dictionary. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementServiceEvents.cs`, line 7.

11. **[IMPLEMENTATION] RarityCalculationService.RecalculateRarityAsync uses SaveAsync without optimistic concurrency (T9 Multi-Instance Safety)**: At line 136 of `RarityCalculationService.cs`, the service reads a definition, modifies `RarityPercent` and `RarityCalculatedAt`, then saves with `SaveAsync` (no ETag check). If multiple instances run the RarityCalculationService simultaneously, they could overwrite each other's concurrent modifications to the definition (e.g., an EarnedCount increment happening at the same time). Per IMPLEMENTATION TENETS, services must use atomic state operations for consistency. File: `/home/lysander/repos/bannou/plugins/lib-achievement/RarityCalculationService.cs`, line 136.

12. **[QUALITY] Operation entry logging uses Information level instead of Debug (T10 Logging Standards)**: Per QUALITY TENETS, operation entry should be logged at Debug level (log input parameters). However, all endpoint methods in `AchievementService.cs` log their entry at `LogInformation` level (lines 104, 181, 219, 303, 382, 429, 518, 674, 813, 895, 1102). T10 states "Operation Entry (Debug): Log input parameters". File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`.

13. **[QUALITY] LogError called with string message but no exception object in SyncPlatformAchievementsAsync (T10 Logging Standards)**: At lines 974, 999, and 1031 of `AchievementService.cs`, and lines 1269, 1315 of the same file, `_logger.LogError(errorMessage)` or `_logger.LogError("message template", ...)` is called without an `Exception` parameter. Per T10 guidance, Error-level logging should include exception context. Some of these are business logic errors (missing definition, missing platform ID) which may not warrant Error level at all. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 974, 999, 1031, 1269, 1315.

14. **[QUALITY] Missing XML documentation on private helper methods (T19 XML Documentation)**: The following private methods lack XML `<summary>` documentation: `PublishPlatformSyncEventAsync` (line 1439, has no summary), `ExecutePlatformUnlockWithRetriesAsync` (line 1381, has no summary). While T19 specifically mentions "all public classes, interfaces, methods", several private helpers in this file DO have documentation, making the inconsistency notable. The `PublishPlatformSyncEventAsync` and `ExecutePlatformUnlockWithRetriesAsync` methods are complex enough to warrant documentation. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 1381 and 1439.

15. **[IMPLEMENTATION] Constructor does not null-check dependencies with ArgumentNullException (T6 Service Implementation Pattern)**: The `AchievementService` constructor (lines 51-68) assigns dependencies directly without null checks (`_messageBus = messageBus;` etc.) unlike the pattern shown in T6 which uses `_messageBus = messageBus ?? throw new ArgumentNullException(nameof(messageBus));`. The `RarityCalculationService` constructor correctly uses null checks. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 60-65.

16. **[IMPLEMENTATION] Error events emitted for expected business conditions in event handlers (T7 Error Handling)**: In `AchievementServiceEvents.cs`, when `UpdateAchievementProgressAsync` or `UnlockAchievementAsync` returns a non-OK status (lines 118-137, 223-242, 334-361), the handler calls `TryPublishErrorAsync`. However, these status codes could include 400 (BadRequest) for validation failures or 409 (Conflict) for lock contention -- both expected conditions. Per IMPLEMENTATION TENETS, error events should NOT be emitted for validation errors (400), not found (404), or conflicts (409). The handlers treat all non-OK statuses as errors. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementServiceEvents.cs`, lines 118-137, 223-242, 334-361.

17. **[IMPLEMENTATION] GameServiceId stored as string in the game-services index set (T25 Internal Model Type Safety)**: At line 150 of `AchievementService.cs`, `body.GameServiceId.ToString()` is used to add the Guid to a set. In `RarityCalculationService.cs` line 113, it is parsed back with `Guid.TryParse(gameServiceIdStr, ...)`. The index stores Guid values as strings because `AddToSetAsync` takes string values, but this is a boundary issue with the state store API rather than a model type safety violation. However, the pattern of `.ToString()` for a Guid when it could potentially be stored as a typed value is worth noting. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, line 150 and `RarityCalculationService.cs`, line 113.

18. **[QUALITY] Missing XML `<param>` tags on public handler methods (T19 XML Documentation)**: The public handler methods `HandleScoreUpdatedAsync`, `HandleMilestoneReachedAsync`, and `HandleRankChangedAsync` in `AchievementServiceEvents.cs` have `<summary>` and `<param>` tags, which is good. However, `RegisterServicePermissionsAsync` at line 1678 of `AchievementService.cs` is missing a `<param>` tag for the `appId` parameter. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, line 1678.

19. **[QUALITY] Missing XML documentation on internal POCO properties (T19 XML Documentation)**: The internal classes `AchievementDefinitionData`, `EntityProgressData`, and `AchievementProgressData` (lines 1692-1737 of `AchievementService.cs`) have class-level `<summary>` tags but none of their properties have XML documentation. While these are `internal` classes, T19 says "All public classes, interfaces, methods, and properties MUST have XML documentation" -- the internal visibility reduces the severity but these properties represent important domain concepts. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 1694-1713, 1720-1724, 1731-1736.

20. **[IMPLEMENTATION] `StateOptions.Ttl` receives `int` (seconds) directly instead of `TimeSpan` (T21 Configuration-First)**: At lines 615 and 765, the code passes `_configuration.ProgressTtlSeconds` directly to `StateOptions.Ttl`. The value is an `int` representing seconds from configuration. This is a potential type safety concern if `StateOptions.Ttl` expects a `TimeSpan` -- the code should be `TimeSpan.FromSeconds(_configuration.ProgressTtlSeconds)`. This may or may not compile depending on the `StateOptions.Ttl` type definition. File: `/home/lysander/repos/bannou/plugins/lib-achievement/AchievementService.cs`, lines 615 and 765.
