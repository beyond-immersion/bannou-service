...
1. **Cross-seed capability aggregation.** **Confirmed** (2026-03-19): Keep `CrossSeedPollinationFactor` as an **independent knob** — it is a read-time manifest computation, distinct from Seed's `SameOwnerGrowthMultiplier` (permanent growth-time state modification). Merge at **capability code** level, not domain level: if multiple seed types produce the same capability code, Agency takes `max(primary, other × factor)`. Does NOT depend on #354 (cross-type domain mappings). Works as soon as Agency is implemented using shared capability codes across seed types. See [#610](https://github.com/beyond-immersion/bannou-service/issues/610). Additionally, Agency needs **base + overlay** manifest composition for character-owned seeds (#676): guardian seed capabilities as persistent base, active character's seed capabilities as full-fidelity overlay added/removed on character possession change. The overlay provides capabilities unique to the character's seed (e.g., dungeon UX modules) while the base provides the spirit's accumulated agency.
<!-- AUDIT:CONFIRMED:2026-03-19:https://github.com/beyond-immersion/bannou-service/issues/610 -->
