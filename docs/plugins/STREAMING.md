# Streaming Plugin Deep Dive

> **Plugin**: lib-streaming
> **Schema**: schemas/streaming-api.yaml
> **Version**: 1.0.0
> **State Stores**: streaming-sessions (MySQL), streaming-audience-pool (Redis), streaming-audience-followers (MySQL), streaming-hype-trains (Redis), streaming-session-voice (Redis), streaming-lock (Redis)
...
**Composability**: Stream session identity and audience simulation are owned here. Platform integration is lib-stream (L3). Voice rooms are lib-voice (L3). Streamer career growth is Seed (`streamer` seed type). Streaming milestone unlocks are Collection. Virtual tips are Currency (`stream_tip` currency type). Sponsorship deals are Contract. Streamer-follower bonds are Relationship. lib-streaming orchestrates the metagame connecting these primitives.

**The divine actor parallel**: The streaming metagame follows the same structural pattern as lib-divine -- an L4 orchestration layer that composes existing Bannou primitives (Seed, Currency, Collection, Contract, Relationship) to deliver game mechanics. Where lib-divine orchestrates blessings and divinity economy, lib-streaming orchestrates audience dynamics and streamer career. They are parallel orchestration layers composing the same underlying primitives, not the same service. This mirrors how Quest and Escrow both compose Contract but provide different game-flavored APIs.

**Simulated audiences are always available**: lib-streaming works without lib-stream (L3) entirely. When no real platform data is available, the service operates on 100% simulated audiences. When lib-stream is available, real audience sentiment pulses are blended seamlessly into the simulated pool. This makes the metagame testable, deployable, and playable without any external platform dependency.
...
**Realm-specific manifestation**: In Omega (cyberpunk meta-dashboard), streaming is explicit -- players see audience stats, manage their stream, and compete with other streamers. In Arcadia, the same mechanics manifest as "performing for a crowd" -- a bard performing at a tavern, a gladiator entertaining an arena, a craftsman demonstrating mastery. The underlying system is identical; the UX presentation varies by realm. lib-streaming provides the mechanics; the client renders realm-appropriate UX.

**Zero Arcadia-specific content**: lib-streaming is a generic audience simulation and streamer career service. Which audience personality types exist, how hype trains escalate, and what streaming milestones unlock are all configured through seed types, collection types, and configuration, not baked into lib-streaming.
...
AudienceMember:
  ...
  trackingId: Guid?                   # For real-derived: matches the lib-stream tracking ID
...
4. **Real blending**: When lib-stream sentiment pulses arrive, they are translated into "real-derived" audience members that blend into the pool indistinguishably from simulated members.
...
5. **Completion**: Trains end via decay, timeout, or session end. Completion publishes `streaming.hype.completed` with peak level and duration.
...
| Level 3 | `streaming.milestone.reached` (notable audience moment) |
...
When lib-stream (L3) publishes `stream.audience.pulse` events, lib-streaming translates them into audience member state changes:
...
4. **Graceful degradation**: If lib-stream stops publishing pulses (platform disconnects, service restarts), the simulated audience continues normally. Real-derived members age out via the standard TTL mechanism.
...
|------------|-------|-----------------------|
| lib-stream (`IStreamClient`) | Associate platform sessions with in-game streaming sessions | Real audience blending unavailable; 100% simulated audiences |
...
| lib-seed (`ISeedClient`) | Streamer career progression (`streamer` seed type) | No career progression; sessions work without career tracking |
| lib-collection (`ICollectionClient`) | Streaming milestone unlocks (first stream, follower milestones, hype achievements) | No milestone unlocks; sessions work without collection integration |
| lib-currency (`ICurrencyClient`) | Virtual tip economy (`stream_tip` currency type) | No tipping; audience engagement works without currency |
| lib-contract (`IContractClient`) | Sponsorship deals (NPC sponsors, milestone-based contracts) | No sponsorship mechanics; sessions work without contracts |
| lib-relationship (`IRelationshipClient`) | Persistent follower bonds (streamer ↔ audience member entity pairs) | Followers tracked in local state only; no cross-service relationship graph |
...
|-----------|-------------|
| *(none yet)* | Streaming is a new L4 service with no current consumers. Future dependents: Gardener (streaming as a garden activity type), Achievement (subscribe to `streaming.milestone.reached` for streaming-related trophies), Leaderboard (subscribe to `streaming.session.ended` for streamer rankings) |
...
### Session Store
**Store**: `streaming-sessions` (Backend: MySQL)

| Key Pattern | Data Type | Purpose |
|-------------|-----------|---------|
| `sess:{streamingSessionId}` | `StreamingSessionModel` | Primary lookup by ID. Stores streamer entity (type+ID), game service scope, status (active/paused/ended), start time, audience metrics, content tags, linked platform session ID (nullable), linked voice room ID (nullable). |
| `sess-streamer:{entityType}:{entityId}` | `StreamingSessionModel` | Active session lookup by streamer entity |
| `sess-game:{gameServiceId}` | `StreamingSessionModel` | Active sessions by game service (for audience routing) |

### Audience Pool Store
**Store**: `streaming-audience-pool` (Backend: Redis, prefix: `streaming:aud`)

| Key Pattern | Data Type | Purpose |
|-------------|-----------|---------|
| `aud:{streamingSessionId}:{memberId}` | `AudienceMemberModel` | Individual audience member state within a session |
| `aud-counts:{streamingSessionId}` | `AudienceCountsModel` | Denormalized counts: total, engaged (>0.5), disengaged (<0.2), real-derived, by-personality-type |
| `aud-tags:{streamingSessionId}` | `ContentTagsModel` | Current session content tags (updated by game event ingestion) |

### Follower Store
**Store**: `streaming-audience-followers` (Backend: MySQL)
...
### Hype Train Store
**Store**: `streaming-hype-trains` (Backend: Redis, prefix: `streaming:hype`)

| Key Pattern | Data Type | Purpose |
|-------------|-----------|---------|
| `hype:{streamingSessionId}` | `HypeTrainModel` | Active hype train for a session (at most one per session). Stores level, score, thresholds, decay rate, trigger category, timestamps. |

### Session-Voice Mapping Store
**Store**: `streaming-session-voice` (Backend: Redis, prefix: `streaming:voice`)

| Key Pattern | Data Type | Purpose |
|-------------|-----------|---------|
| `voice:{streamingSessionId}` | `SessionVoiceMappingModel` | Maps streaming session to voice room ID (for voice room lifecycle orchestration) |
...
### Distributed Locks
**Store**: `streaming-lock` (Backend: Redis, prefix: `streaming:lock`)

| Key Pattern | Purpose |
|-------------|---------|
| `streaming:lock:session:{streamingSessionId}` | Session mutation lock (create, update, end) |
| `streaming:lock:hype:{streamingSessionId}` | Hype train mutation lock |
| `streaming:lock:audience-tick` | Audience simulation tick singleton lock |
| `streaming:lock:career-worker` | Career progression worker singleton lock |
...
|-------|-----------|---------|
| `streaming.session.started` | `StreamingSessionStartedEvent` | In-game streaming session created |
| `streaming.session.ended` | `StreamingSessionEndedEvent` | Streaming session ended (with audience metrics, peak viewers, duration, hype summary) |
| `streaming.session.paused` | `StreamingSessionPausedEvent` | Streaming session paused (streamer AFK, scene transition) |
| `streaming.session.resumed` | `StreamingSessionResumedEvent` | Streaming session resumed |
| `streaming.hype.started` | `StreamingHypeStartedEvent` | Hype train started (level 1) |
| `streaming.hype.leveled` | `StreamingHypeLeveledEvent` | Hype train advanced to new level |
| `streaming.hype.completed` | `StreamingHypeCompletedEvent` | Hype train ended (with peak level, duration, trigger category) |
| `streaming.milestone.reached` | `StreamingMilestoneReachedEvent` | Streamer career milestone (first stream, follower count, watch hours, world-first discovery) |
| `streaming.audience.changed` | `StreamingAudienceChangedEvent` | Significant audience composition change (burst join, mass exodus, sentiment shift) |

### Consumed Events

| Topic | Handler | Action |
|-------|---------|--------|
| `stream.audience.pulse` | `HandleSentimentPulseAsync` | Translate real audience sentiment into real-derived audience members, feed hype train calculations. (Soft -- no-op if lib-stream absent) |
| `stream.platform.session.started` | `HandlePlatformSessionStartedAsync` | If associated with an in-game session, start real audience blending. (Soft -- no-op if lib-stream absent) |
...
| `streaming.session.ended` | `HandleStreamingSessionEndedAsync` | Trigger career progression: record seed growth, evaluate milestones, grant collection entries, update follower relationships |

### Resource Cleanup (T28)

| Target Resource | Source Type | On Delete | Cleanup Endpoint |
|----------------|-------------|-----------|-----------------|
| character | streaming | CASCADE | `/streaming/cleanup-by-character` |
| game-service | streaming | CASCADE | `/streaming/cleanup-by-game-service` |
...
|----------|---------|---------|---------|
| `StreamingEnabled` | `STREAMING_ENABLED` | `false` | Master feature flag |
| `StreamerSeedTypeCode` | `STREAMING_STREAMER_SEED_TYPE_CODE` | `streamer` | Seed type code for streamer career growth |
| `TipCurrencyCode` | `STREAMING_TIP_CURRENCY_CODE` | `stream_tip` | Currency code for virtual tip economy |
| `MilestoneCollectionType` | `STREAMING_MILESTONE_COLLECTION_TYPE` | `streaming_milestones` | Collection type code for streaming milestone unlocks |
| `HypeCollectionType` | `STREAMING_HYPE_COLLECTION_TYPE` | `streaming_hype` | Collection type code for hype achievements |
| `WorldFirstCollectionType` | `STREAMING_WORLD_FIRST_COLLECTION_TYPE` | `streaming_world_first` | Collection type code for world-first stream unlocks |
| `SponsorshipContractTemplateCode` | `STREAMING_SPONSORSHIP_CONTRACT_TEMPLATE_CODE` | `stream_sponsorship` | Contract template code for NPC sponsorship deals |
| `FollowerRelationshipTypeCode` | `STREAMING_FOLLOWER_RELATIONSHIP_TYPE_CODE` | `stream_follower` | Relationship type code for streamer-follower bonds |
| `DefaultAudiencePoolSize` | `STREAMING_DEFAULT_AUDIENCE_POOL_SIZE` | `50` | Base audience pool size for new/Unknown phase streamers |
| `AudiencePoolScalePerPhase` | `STREAMING_AUDIENCE_POOL_SCALE_PER_PHASE` | `2.0` | Multiplier applied per career phase (Rising=2x, Popular=4x, etc.) |
| `AudienceTickIntervalSeconds` | `STREAMING_AUDIENCE_TICK_INTERVAL_SECONDS` | `5` | How often the audience simulation evaluates engagement and migration |
| `EngagementDecayBaseRate` | `STREAMING_ENGAGEMENT_DECAY_BASE_RATE` | `0.02` | Per-tick engagement decay when content doesn't match interests |
| `HypeTrainTriggerThreshold` | `STREAMING_HYPE_TRAIN_TRIGGER_THRESHOLD` | `10` | Minimum matching sentiments in a pulse interval to trigger a hype train |
| `HypeTrainLevelExponent` | `STREAMING_HYPE_TRAIN_LEVEL_EXPONENT` | `1.5` | Exponential scaling factor for level thresholds |
| `HypeTrainDecayRate` | `STREAMING_HYPE_TRAIN_DECAY_RATE` | `0.1` | Per-tick decay rate for hype train score |
| `HypeTrainMaxDurationMinutes` | `STREAMING_HYPE_TRAIN_MAX_DURATION_MINUTES` | `10` | Hard timeout for hype trains |
| `FollowerEngagementThreshold` | `STREAMING_FOLLOWER_ENGAGEMENT_THRESHOLD` | `0.6` | Minimum average engagement for an audience member to become a follower |
| `FollowerWatchTimeMinutes` | `STREAMING_FOLLOWER_WATCH_TIME_MINUTES` | `30` | Minimum cumulative watch time for follower conversion |
| `MaxFollowersPerStreamer` | `STREAMING_MAX_FOLLOWERS_PER_STREAMER` | `10000` | Maximum followers per streamer entity |
| `RealDerivedMemberTtlSeconds` | `STREAMING_REAL_DERIVED_MEMBER_TTL_SECONDS` | `30` | TTL for anonymous real-derived audience members (2x pulse interval) |
| `CareerWorkerIntervalSeconds` | `STREAMING_CAREER_WORKER_INTERVAL_SECONDS` | `60` | Career progression worker evaluation frequency |
| `AutoStreamOnGameSession` | `STREAMING_AUTO_STREAM_ON_GAME_SESSION` | `false` | Automatically create streaming sessions when game sessions start |
| `AutoVoiceOnGameSession` | `STREAMING_AUTO_VOICE_ON_GAME_SESSION` | `true` | Automatically create voice rooms when game sessions start |
| `GrowthDebounceIntervalMs` | `STREAMING_GROWTH_DEBOUNCE_INTERVAL_MS` | `5000` | Debounce interval for seed growth contributions (prevent overwhelming lib-seed) |
| `DistributedLockTimeoutSeconds` | `STREAMING_DISTRIBUTED_LOCK_TIMEOUT_SECONDS` | `30` | Timeout for distributed lock acquisition |
...
| Service | Role |
|---------|------|
| `ILogger<StreamingService>` | Structured logging |
| `StreamingServiceConfiguration` | Typed configuration access (25 properties) |
...
|--------|---------|-----------------|----------|
| `AudienceTickWorker` | Evaluates engagement decay, migration, and interest matching for all active sessions | `AudienceTickIntervalSeconds` (5s) | `streaming:lock:audience-tick` |
| `CareerProgressionWorker` | Evaluates milestone thresholds, grants collection entries, records seed growth for ended sessions | `CareerWorkerIntervalSeconds` (60s) | `streaming:lock:career-worker` |
...
- **Start** (`/streaming/session/start`): Validates streamer entity and game service exist. Creates `StreamingSessionModel` in MySQL. Allocates initial audience pool from template reservoir (size based on career seed phase). Optionally associates with a platform session via lib-stream. Optionally creates voice room via lib-voice. Publishes `streaming.session.started`.
- **End** (`/streaming/session/end`): Lock. Ends audience simulation. Calculates session metrics (total watch-time, peak viewers, average engagement, hype summary). Evaluates follower conversions. Destroys audience pool from Redis. Deletes voice room via lib-voice if created. Records seed growth contributions. Publishes `streaming.session.ended`.
- **Pause** (`/streaming/session/pause`): Pauses audience simulation (decay continues but no new engagement). Publishes `streaming.session.paused`.
- **Resume** (`/streaming/session/resume`): Resumes audience simulation. Publishes `streaming.session.resumed`.
- **Get** (`/streaming/session/get`): Returns session state including current audience metrics, active hype train, content tags, linked platform session.
- **List** (`/streaming/session/list`): Active and recent sessions by game service or streamer entity, paginated.

### Audience Management (4 endpoints)

All endpoints require `developer` role.

- **GetAudienceSnapshot** (`/streaming/audience/snapshot`): Returns current audience composition: total count, engaged/disengaged counts, personality type distribution, real-derived count (admin-only), top engaged members.
- **GetAudienceFollowers** (`/streaming/audience/followers`): Paged query of persistent followers for a streamer entity. Includes engagement history, first-followed date, total watch time.
- **InjectAudienceEvent** (`/streaming/audience/inject`): Debug endpoint to manually inject a sentiment or audience event into a session. For testing hype trains and audience dynamics without real platform data.
- **SetContentTags** (`/streaming/audience/set-tags`): Manually set content tags for a session (normally derived from game events). For testing interest matching.

### Hype Train Management (2 endpoints)

All endpoints require `developer` role.

- **GetHypeStatus** (`/streaming/hype/status`): Returns current hype train state for a session (level, score, decay rate, time remaining).
- **InjectHypeEvent** (`/streaming/hype/inject`): Debug endpoint to inject a hype-contributing event. For testing hype train escalation.

### Career / Composability (3 endpoints)

All endpoints require `developer` role.

- **GetCareer** (`/streaming/career/get`): Returns streamer career summary: seed phase, growth domain breakdown, milestone history, follower count, total sessions, total watch hours.
- **GetMilestones** (`/streaming/career/milestones`): Returns all achieved milestones for a streamer entity with timestamps and session context.
- **RecordGrowth** (`/streaming/career/record-growth`): Manually record seed growth contribution. For testing career progression.

### Voice Room Orchestration (2 endpoints)

All endpoints require `developer` role.

- **CreateVoiceRoom** (`/streaming/voice/create`): Creates a voice room via lib-voice and stores the session-voice mapping. Called by event handler on `game-session.created`, or manually for ad-hoc voice rooms.
- **DeleteVoiceRoom** (`/streaming/voice/delete`): Deletes a voice room via lib-voice and clears the mapping. Called by event handler on `game-session.ended`, or manually.

### Cleanup Endpoints (2 endpoints)

Resource-managed cleanup via lib-resource (per FOUNDATION TENETS):

- **CleanupByCharacter** (`/streaming/cleanup-by-character`): End active sessions where streamer is the character. Archive follower records. Remove streaming collection entries referencing this character.
- **CleanupByGameService** (`/streaming/cleanup-by-game-service`): End all active sessions for the game service. Delete all session records, follower records, and streaming-specific collection entries.
...
Detailed in the Seed Type section above. The `streamer` seed type provides progressive growth that gates audience pool size, hype train availability, sponsorship eligibility, and milestone thresholds.

### Collection: Streaming Unlocks

| Collection Category | Example Entries | Trigger Event |
|-------------------|-----------------|---------------|
| Streaming Milestones | "First Stream", "100 Followers", "1000 Watch Hours", "10 Sessions" | `streaming.milestone.reached` |
| Hype Achievements | "First Hype Train", "Level 3 Hype", "Level 5 Hype", "Legendary Hype" | `streaming.hype.completed` |
| World-First Streams | "Discovered [X] On Stream" | `streaming.milestone.reached` with WorldFirstCount > 0 |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                  Streaming Service: The Audience Metagame                  │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
...
│  │  (always present)            │  (when lib-stream L3 present)  │      │
│  │                              │                                │      │
│  │  ┌─────┐ ┌─────┐ ┌─────┐   │  ┌─────┐ ┌─────┐              │      │
...
│  │                                                                │      │
│  │  Level 3+ → streaming.milestone.reached → Character History   │      │
│  │  Level 5+ → streaming.milestone.reached → Realm History       │      │
│  └───────────────────────────────────────────────────────────────┘      │
│           │                                                              │
│           ▼                                                              │
...
lib-streaming (L4) now owns the "game sessions have voice" orchestration that previously lived in GameSession (L2) → Voice (L4), which was a hierarchy violation:

### Before (Hierarchy Violation)
```
GameSession (L2) ──calls──► Voice (L4)   ← FORBIDDEN (L2 → L4)
```
...
GameSession (L2) ──publishes──► game-session.created event
                                        │
                                        ▼
lib-streaming (L4) ──subscribes──► HandleGameSessionCreatedAsync
                                        │
                                        ▼
lib-streaming (L4) ──calls──► lib-voice (L3)   ← VALID (L4 → L3)
    Creates voice room, stores mapping
                                        │
game-session.ended event ───────────────┘
                                        │
                                        ▼
lib-streaming (L4) ──calls──► lib-voice (L3)
    Deletes voice room, clears mapping
```

This eliminates the hierarchy violation entirely. GameSession publishes events; lib-streaming consumes them and orchestrates the voice+streaming stack.
...
- Create streaming-api.yaml schema with all endpoints (19 endpoints across 5 groups)
- Create streaming-events.yaml schema (9 published events, 8 consumed events)
- Create streaming-configuration.yaml schema (25 configuration properties)
- Create streaming-client-events.yaml (hype train client events, audience milestone notifications)
...
6. **Client events for streaming UX**: `streaming-client-events.yaml` for pushing audience reactions, hype train progress, milestone notifications, and follower updates to the streamer's WebSocket client.

7. **Variable Provider Factory**: `IStreamingVariableProviderFactory` for ABML behavior expressions (`${streaming.audience_size}`, `${streaming.hype_level}`, `${streaming.career_phase}`). Could enable NPCs to react to streaming activity (a bard NPC performs harder when the audience is large).
...
1. **Audience personality types are opaque strings**: Not an enum. Follows the same extensibility pattern as seed type codes, collection type codes, faction codes. Different games define different audience personality types. lib-streaming stores whatever personality string is provided and uses configurable matching weights.
...
8. **Voice room orchestration is a stopgap**: lib-streaming owns the game-session → voice room flow to eliminate the GameSession → Voice hierarchy violation. Long-term, this orchestration may move to a dedicated "session orchestrator" if more cross-service session setup logic accumulates. For now, lib-streaming is the natural home because it already subscribes to game-session events.
...
7. **Voice room broadcasting consent UX**: When lib-streaming creates a voice room and a player wants to broadcast it, the consent flow goes through lib-voice. But the UX decision (modal dialog? in-room notification? veto-based?) is a client-side design question that affects how consent is presented and collected. lib-streaming/lib-voice provide the API; the client determines the UX.
