...
| lib-resource (`IResourceClient`) | Reference tracking checks before deletion; cleanup coordination; compression callback registration for location archival (provides base location data including parent context and children summary) |
...
- **DeprecateLocation** (`/location/deprecate`): Sets `IsDeprecated=true` with timestamp and reason. Idempotent — returns OK with current state if already deprecated (no event published). Location remains queryable. Publishes update event on state change.
- **UndeprecateLocation** (`/location/undeprecate`): Restores deprecated location. Idempotent — returns OK with current state if not deprecated (no event published). Publishes update event on state change.
- **GetLocationCompressData** (`/location/get-compress-data`): Compression callback for lib-resource archival. Returns `LocationBaseArchive` containing base location data, parent context (one level up: name, code, type), and children summary (count + codes). Requires location to be deprecated (BadRequest if not). Developer role only.
...
- **2026-03-04**: Added compression/archival support via `x-compression-callback`. Location is now a first-class resource-tracked entity with `GetLocationCompressData` endpoint providing base location data, parent context (one level up), and children summary for the content flywheel. Realm provides contextual data to location archives via the multi-callback pattern. Relationship `x-references` now covers Location entity types alongside Character and Realm.
