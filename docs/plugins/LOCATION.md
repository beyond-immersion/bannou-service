...
3. ~~**No realm deletion reference tracking via lib-resource**~~ ([#590](https://github.com/beyond-immersion/bannou-service/issues/590)): **COMPLETED** (2026-03-15) — Location now declares `x-references` with `target: realm` and `onDelete: restrict` in its schema. RESTRICT references are registered with lib-resource during location creation and transfer. Realm deletion is blocked if locations exist in that realm. A `migrate-by-realm` endpoint is available for lib-resource to call during realm merge, transferring locations from the source realm to the target realm.
