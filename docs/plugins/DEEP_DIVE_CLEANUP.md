...
| Contract | DONE | Fixed silent catch-all (T7 specific exceptions). Fixed T7 ApiException handling for mesh calls. T21/T5 issues require planning (config scoping, schema events). Removed T19 false positives. | PHASE2: Fixed 2 (catch-all, T7 ApiException), remaining: T21 cache scoping, T21 enum config, T5 events |
