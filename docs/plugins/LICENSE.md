...
| `license.board-template.deleted` | `LicenseBoardTemplateDeletedEvent` | Unused Category B infrastructure — board templates are never deleted. Generated by `x-lifecycle` for future safe deletion pattern. |
...
`ListBoardTemplatesAsync` supports paginated listing filtered by `GameServiceId` using `QueryAsync` with configurable page size. Supports `includeDeprecated` parameter (default false, per IMPLEMENTATION TENETS).
