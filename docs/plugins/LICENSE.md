...
| `license.board-template.deleted` | `LicenseBoardTemplateDeletedEvent` | Unused Category B infrastructure — board templates are never deleted (T31). Generated by `x-lifecycle` for future safe deletion pattern. |
...
- **No template deletion**: Board templates can only be deprecated, never deleted. This is standard Category B behavior per IMPLEMENTATION TENETS deprecation lifecycle — templates whose instances outlive them are terminal-deprecation-only. The `*.deleted` lifecycle event exists as infrastructure for the future safe deletion pattern (see GitHub Issue #611).
