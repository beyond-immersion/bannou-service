...
| **Action gate** | Director writes gated action types to `director-overrides` Redis store; `DirectorOverrideProviderFactory` returns prohibitively high GOAP cost for gated actions via `${director.gate.<actionType>}` | GOAP planner naturally avoids gated actions; developer "approves" by clearing gates; stale gates auto-cleared after `ActionGateTimeoutSeconds` (fail-open) |
...
1. **Tap data protocol design**: Three-tier delivery model — perceptions via batched events (same `EventBatcher` pattern as Item/Affix), GOAP plan changes via normal low-frequency events, variable snapshots via pull-based polling (developer calls `GET /director/actor/get-variables` which reads from the existing `DirectorOverrideProviderFactory` cache). Remaining open items: exact batched perception event format for `director-client-events.yaml`, batch interval configuration. See [#609](https://github.com/beyond-immersion/bannou-service/issues/609).
<!-- AUDIT:NEEDS_DESIGN:2026-03-08:https://github.com/beyond-immersion/bannou-service/issues/609 -->

2. ~~**Drive session checkpoint mechanism**~~: **FIXED** (2026-03-17) - Resume from `on_tick` with no checkpoint tracking. Each tick is a complete evaluation cycle; the developer's state changes (feelings, goals, memories) are the persistent state the autonomous system reads. Multi-step flows left in-progress self-heal via Contract TTL. ABML transactional blocks (Actor [#692](https://github.com/beyond-immersion/bannou-service/issues/692)) will allow declaring atomic action sequences that refuse interruption by drive binding.

3. ~~**Action gate approval mechanism design**~~: **FIXED** (2026-03-17) - Reframed as GOAP cost modifiers via existing Variable Provider Factory. `SetActionGates` writes to `ovr:{actorId}:gates`; `DirectorOverrideProviderFactory` returns prohibitively high cost for gated actions via `${director.gate.<actionType>}`; GOAP planner naturally avoids them. Developer "approves" by clearing gates. Timeout auto-approve via `DirectorSessionTimeoutWorker`. No approval RabbitMQ topic, no new DI interface, zero Actor changes.
...
- **Action gate mechanism resolved** (2026-03-17): Design Consideration #3 resolved — gates reframed as GOAP cost modifiers via existing `DirectorOverrideProviderFactory`. `${director.gate.<actionType>}` returns prohibitive cost; GOAP planner naturally avoids gated actions. Developer "approves" by clearing gates. No approval RabbitMQ topic, no new Actor infrastructure. Issue [#668](https://github.com/beyond-immersion/bannou-service/issues/668) closed.
- **Drive checkpoint mechanism resolved** (2026-03-17): Design Consideration #2 resolved — resume from `on_tick` with no checkpoint tracking. Developer state changes persist; autonomous system reads them on next tick. ABML transactional blocks (Actor [#692](https://github.com/beyond-immersion/bannou-service/issues/692)) created as a separate Actor feature for atomic action sequences. Issue [#612](https://github.com/beyond-immersion/bannou-service/issues/612) closed.
- **Tap protocol partially resolved** (2026-03-17): Design Consideration #1 updated — three-tier model confirmed: batched perception events, normal GOAP plan events, pull-based variable snapshots. Remaining: client event schema format (Phase 7). Issue [#609](https://github.com/beyond-immersion/bannou-service/issues/609) updated.
