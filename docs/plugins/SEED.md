...
- **Seed → Collection reverse pipeline** ([#700](https://github.com/beyond-immersion/bannou-service/issues/700)): The Collection → Seed direction is implemented (`SeedCollectionUnlockListener` matches entry tags against `collectionGrowthMappings`), but the reverse — seed growth thresholds automatically granting collection entries — is not. Add `capabilityCollectionGrants` to seed type definitions mapping capability codes to collection type + entry code + tags. When growth recording triggers a capability unlock, Seed calls `ICollectionClient.GrantEntryAsync` inline (lazy check, no background worker). The granted entry's tags then flow through the existing `SeedCollectionUnlockListener` on all other seeds owned by the same entity, completing the cross-seed pollination feedback loop described in the FAQ. Seed and Collection are both L2 (guaranteed co-located); `GrantEntryAsync` is idempotent (natural convergence).
<!-- AUDIT:NEEDS_DESIGN:2026-03-18:https://github.com/beyond-immersion/bannou-service/issues/700 -->
...
- [#700](https://github.com/beyond-immersion/bannou-service/issues/700) - Seed → Collection reverse pipeline: capability-gated collection grants for cross-seed pollination feedback loop
