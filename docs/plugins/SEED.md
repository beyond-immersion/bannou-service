...
3. **Missing TransferGrowth endpoint for household split**: Seed must provide a `POST /seed/transfer-growth` endpoint that transfers a proportion of accumulated growth from one seed to another. Called by Arbitration's `SeedGrowthTransfer` consequence during household dissolution enforcement. Takes `sourceSeedId`, `targetSeedId`, and `proportion` (0.0–1.0). Must be idempotent for Arbitration's self-healing retry. Design decision made in #436; implementation tracked in #671.
<!-- AUDIT:NEEDS_DESIGN:2026-03-16:https://github.com/beyond-immersion/bannou-service/issues/671 -->
