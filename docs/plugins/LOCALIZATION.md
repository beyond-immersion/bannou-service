...
7. ~~**ABML `ILocalizationProvider` bridge**~~: **IMPLEMENTED** (2026-04-26, issue #689) — `LocalizationServiceSource` plugs into the behavior system's `ILocalizationSource` extension point at priority 100. The remaining work is wiring an actual consumer of `ILocalizationProvider.GetText` — `${localization()}` is not currently registered as an ABML built-in function. **Tracked in issue #726**.
...
4. ~~**Relationship to existing ABML `ILocalizationProvider`**~~: **RESOLVED** (2026-04-26) — Implemented as `LocalizationServiceSource : ILocalizationSource` in `plugins/lib-localization/Services/`. The bridge plugs into `bannou-service`'s existing source-pluggable architecture: `FileLocalizationProvider` (in lib-behavior) was modified to auto-discover sources via `IEnumerable<ILocalizationSource>` constructor injection, and lib-behavior also registers a `BehaviorEmbeddedLocalizationSource` (priority 50) that loads YAML strings from assembly embedded resources via the new `EmbeddedYamlLocalizationSource` base class (in `bannou-service/Behavior/`, mirroring the `EmbeddedResourceProvider` pattern). The service-backed source runs at priority 100, so centralized entries override embedded fallbacks when both are present. Cache invalidation via dual event subscriptions ensures multi-node correctness: `localization.category.updated` invalidates the single affected language, while `localization.category.deleted` invalidates all cached bundles (the cascade affects every language the category had entries in, and the event payload does not enumerate them). **Important caveat**: nothing in production code calls `ILocalizationProvider.GetText` today — `${localization()}` is not a registered ABML built-in function and `DialogueResolver` does not consult `ILocalizationProvider`. Issue #726 tracks the consumer-side wiring (function registration or `DialogueResolver` augmentation).
...
- **2026-04-26**: Issue #689 — Bridged lib-localization with ABML's `ILocalizationProvider`. Implemented `LocalizationServiceSource : ILocalizationSource` (priority 100) backed by `ExportLocalizationAsync` with per-locale bundle caching (TTL = `CacheExpirationMinutes`) and event-based invalidation via `localization.category.updated` subscription. Modified `FileLocalizationProvider` (lib-behavior) to auto-discover `IEnumerable<ILocalizationSource>` from DI. Added `EmbeddedYamlLocalizationSource` abstract base in `bannou-service/Behavior/` (mirrors `EmbeddedResourceProvider` pattern from `bannou-service/Providers/`) and a concrete `BehaviorEmbeddedLocalizationSource` (priority 50) registered by lib-behavior. Filed follow-up #726 for the missing consumer wiring (`${localization()}` ABML function or `DialogueResolver` augmentation).
