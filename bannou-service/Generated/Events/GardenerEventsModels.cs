//----------------------
// <auto-generated>
//     Generated using the NSwag toolchain v14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0)) (http://NSwag.org)
//
//     WARNING: DO NOT EDIT THIS FILE (FOUNDATION TENETS)
//     This file is auto-generated from OpenAPI schemas in /schemas/.
//     Any manual changes will be overwritten on next generation.
//
//     To modify behavior:
//     1. Edit the source schema (schemas/*-api.yaml or schemas/*-events.yaml)
//     2. Run: scripts/generate-all-services.sh
//     3. Implement business logic in the service class (e.g., *Service.cs)
//
//     RELATED FILES:
//     - Request/Response models: bannou-service/Generated/Models/{Service}Models.cs
//     - Event models: bannou-service/Generated/Events/{Service}EventsModels.cs
//     - Lifecycle events: bannou-service/Generated/Events/{Service}LifecycleEvents.cs
//
//     See: docs/reference/tenets/FOUNDATION.md (Schema-First Development, Service Hierarchy)
// </auto-generated>
//----------------------

#nullable enable

using BeyondImmersion.Bannou.Core;
using BeyondImmersion.BannouService;
using BeyondImmersion.BannouService.Gardener;

#pragma warning disable 108 // Disable "CS0108 '{derivedDto}.ToJson()' hides inherited member '{dtoBase}.ToJson()'. Use the new keyword if hiding was intended."
#pragma warning disable 114 // Disable "CS0114 '{derivedDto}.RaisePropertyChanged(String)' hides inherited member 'dtoBase.RaisePropertyChanged(String)'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword."
#pragma warning disable 472 // Disable "CS0472 The result of the expression is always 'false' since a value of type 'Int32' is never equal to 'null' of type 'Int32?'
#pragma warning disable 612 // Disable "CS0612 '...' is obsolete"
#pragma warning disable 649 // Disable "CS0649 Field is never assigned to, and will always have its default value null"
#pragma warning disable 1573 // Disable "CS1573 Parameter '...' has no matching param tag in the XML comment for ...
#pragma warning disable 1591 // Disable "CS1591 Missing XML comment for publicly visible type or member ..."
#pragma warning disable 8073 // Disable "CS8073 The result of the expression is always 'false' since a value of type 'T' is never equal to 'null' of type 'T?'"
#pragma warning disable 3016 // Disable "CS3016 Arrays as attribute arguments is not CLS-compliant"
#pragma warning disable 8600 // Disable "CS8600 Converting null literal or possible null value to non-nullable type"
#pragma warning disable 8602 // Disable "CS8602 Dereference of a possibly null reference"
#pragma warning disable 8603 // Disable "CS8603 Possible null reference return"
#pragma warning disable 8604 // Disable "CS8604 Possible null reference argument for parameter"
#pragma warning disable 8625 // Disable "CS8625 Cannot convert null literal to non-nullable reference type"
#pragma warning disable 8765 // Disable "CS8765 Nullability of type of parameter doesn't match overridden member (possibly because of nullability attributes)."

namespace BeyondImmersion.BannouService.Events;

using System = global::System;

/// <summary>
/// Published when a player enters the garden
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerGardenEnteredEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.garden.entered
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.garden.entered";

    /// <summary>
    /// WebSocket session ID of the player that entered the garden
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// Active seed for this garden session
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("seedId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid SeedId { get; set; } = default!;

    /// <summary>
    /// Created garden instance ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("gardenInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid GardenInstanceId { get; set; } = default!;

}

/// <summary>
/// Published when a player leaves the garden
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerGardenLeftEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.garden.left
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.garden.left";

    /// <summary>
    /// WebSocket session ID of the player that left the garden
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// Garden instance that was destroyed
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("gardenInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid GardenInstanceId { get; set; } = default!;

    /// <summary>
    /// Total garden session duration in seconds
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("sessionDurationSeconds")]
    public float SessionDurationSeconds { get; set; } = default!;

}

/// <summary>
/// Published when a POI spawns in a garden instance
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerPoiSpawnedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.poi.spawned
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.poi.spawned";

    /// <summary>
    /// Garden instance the POI spawned in
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("gardenInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid GardenInstanceId { get; set; } = default!;

    /// <summary>
    /// Spawned POI identifier
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("poiId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid PoiId { get; set; } = default!;

    /// <summary>
    /// Sensory type of the spawned POI
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("poiType")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    [System.Text.Json.Serialization.JsonConverter(typeof(System.Text.Json.Serialization.JsonStringEnumConverter))]
    public PoiType PoiType { get; set; } = default!;

    /// <summary>
    /// Scenario template this POI leads to
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

}

/// <summary>
/// Published when a player enters a POI or triggers a scenario from it
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerPoiEnteredEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.poi.entered
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.poi.entered";

    /// <summary>
    /// WebSocket session ID of the player that entered the POI
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// POI that was entered
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("poiId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid PoiId { get; set; } = default!;

    /// <summary>
    /// Scenario template associated with the POI
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

}

/// <summary>
/// Published when a player declines a POI
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerPoiDeclinedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.poi.declined
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.poi.declined";

    /// <summary>
    /// WebSocket session ID of the player that declined the POI
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// POI that was declined
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("poiId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid PoiId { get; set; } = default!;

    /// <summary>
    /// Scenario template that was declined
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

}

/// <summary>
/// Published when a POI expires without player interaction
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerPoiExpiredEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.poi.expired
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.poi.expired";

    /// <summary>
    /// Garden instance the POI was in
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("gardenInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid GardenInstanceId { get; set; } = default!;

    /// <summary>
    /// POI that expired
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("poiId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid PoiId { get; set; } = default!;

}

/// <summary>
/// Published when a scenario instance is created
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerScenarioStartedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.scenario.started
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.scenario.started";

    /// <summary>
    /// Created scenario instance ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// Template the instance was created from
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

    /// <summary>
    /// Backing game session ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("gameSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid GameSessionId { get; set; } = default!;

    /// <summary>
    /// WebSocket session ID of the player that entered the scenario
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// Active seed for the entering player
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("seedId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid SeedId { get; set; } = default!;

    /// <summary>
    /// Human-readable template code for the scenario
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateCode")]
    public string ScenarioTemplateCode { get; set; } = default!;

    /// <summary>
    /// ABML behavior document ID from the template content, if any (enables Puppetmaster to load scenario-specific behaviors)
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("behaviorDocumentId")]
    public System.Guid? BehaviorDocumentId { get; set; } = default!;

}

/// <summary>
/// Published when a scenario is completed with growth awards
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerScenarioCompletedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.scenario.completed
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.scenario.completed";

    /// <summary>
    /// Completed scenario instance ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// Template the instance was created from
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

    /// <summary>
    /// WebSocket session ID of the player that completed the scenario
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// Growth awarded per domain
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("growthAwarded")]
    [System.ComponentModel.DataAnnotations.Required]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Collections.Generic.IDictionary<string, float> GrowthAwarded { get; set; } = new System.Collections.Generic.Dictionary<string, float>();

}

/// <summary>
/// Published when a scenario is abandoned
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerScenarioAbandonedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.scenario.abandoned
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.scenario.abandoned";

    /// <summary>
    /// Abandoned scenario instance ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// WebSocket session ID of the player that abandoned the scenario
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

}

/// <summary>
/// Published when a player chains from one scenario to another
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerScenarioChainedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.scenario.chained
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.scenario.chained";

    /// <summary>
    /// Scenario instance that was chained from
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("previousScenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid PreviousScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// New scenario instance that was created
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("newScenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid NewScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// WebSocket session ID of the player chaining scenarios
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("webSocketSessionId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid WebSocketSessionId { get; set; } = default!;

    /// <summary>
    /// Current chain depth after chaining
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("chainDepth")]
    public int ChainDepth { get; set; } = default!;

}

/// <summary>
/// Published when bonded players enter a scenario together
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerBondEnteredTogetherEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.bond.entered-together
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.bond.entered-together";

    /// <summary>
    /// Bond linking the participants
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("bondId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid BondId { get; set; } = default!;

    /// <summary>
    /// Shared scenario instance ID
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioInstanceId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioInstanceId { get; set; } = default!;

    /// <summary>
    /// Template the instance was created from
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("scenarioTemplateId")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Guid ScenarioTemplateId { get; set; } = default!;

    /// <summary>
    /// WebSocket session IDs of bond participants entering together
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("participantSessionIds")]
    [System.ComponentModel.DataAnnotations.Required]
    [System.Text.Json.Serialization.JsonRequired]
    public System.Collections.Generic.ICollection<System.Guid> ParticipantSessionIds { get; set; } = new System.Collections.ObjectModel.Collection<System.Guid>();

}

/// <summary>
/// Published when the deployment phase changes
/// </summary>
[System.CodeDom.Compiler.GeneratedCode("NJsonSchema", "14.5.0.0 (NJsonSchema v11.4.0.0 (Newtonsoft.Json v13.0.0.0))")]
public partial class GardenerPhaseChangedEvent : BaseServiceEvent
{

    /// <summary>
    /// Event type identifier: gardener.phase.changed
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("eventName")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    public override string EventName { get; set; } = "gardener.phase.changed";

    /// <summary>
    /// Phase before the change
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("previousPhase")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    [System.Text.Json.Serialization.JsonConverter(typeof(System.Text.Json.Serialization.JsonStringEnumConverter))]
    public DeploymentPhase PreviousPhase { get; set; } = default!;

    /// <summary>
    /// Phase after the change
    /// </summary>
    [System.Text.Json.Serialization.JsonPropertyName("newPhase")]
    [System.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
    [System.Text.Json.Serialization.JsonRequired]
    [System.Text.Json.Serialization.JsonConverter(typeof(System.Text.Json.Serialization.JsonStringEnumConverter))]
    public DeploymentPhase NewPhase { get; set; } = default!;

}



#pragma warning restore  108
#pragma warning restore  114
#pragma warning restore  472
#pragma warning restore  612
#pragma warning restore  649
#pragma warning restore 1573
#pragma warning restore 1591
#pragma warning restore 8073
#pragma warning restore 3016
#pragma warning restore 8600
#pragma warning restore 8602
#pragma warning restore 8603
#pragma warning restore 8604
#pragma warning restore 8625
#pragma warning restore 8765
