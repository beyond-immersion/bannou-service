...
public static class DeprecationCleanupHelper
{
    ...
    /// <summary>
    /// Executes a Category B deprecation cleanup sweep for entities with developer-assigned
    /// string identifiers (e.g., achievement and leaderboard definitions). Identical behavior
    /// to the Guid overload but preserves string key type safety throughout.
    /// </summary>
    /// <typeparam name="TEntity">The internal model type of the deprecated entity.</typeparam>
    /// <param name="deprecatedEntities">All currently deprecated entities to evaluate.</param>
    ...
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Cleanup sweep result with counts and cleaned entity IDs.</returns>
    public static async Task<StringKeyCleanupSweepResult> ExecuteCleanupSweepAsync<TEntity>(
        IReadOnlyList<TEntity> deprecatedEntities,
        ...
        CancellationToken ct)
    {
        ...
        return new StringKeyCleanupSweepResult(cleaned.Count, remaining, errors, cleaned);
    }
}
...
/// <summary>
/// Result of a deprecation cleanup sweep for string-keyed entities. Mapped to the generated
/// CleanDeprecatedStringKeyResponse by services with developer-assigned string identifiers
/// (e.g., achievement and leaderboard definitions).
/// </summary>
/// <param name="Cleaned">Number of deprecated entities removed (or eligible, if dry run).</param>
/// <param name="Remaining">Number of deprecated entities still having active instances or within grace period.</param>
/// <param name="Errors">Number of entities that failed during cleanup processing.</param>
/// <param name="CleanedIds">IDs of successfully cleaned (or eligible) entities.</param>
public record StringKeyCleanupSweepResult(
    int Cleaned,
    int Remaining,
    int Errors,
    IReadOnlyList<string> CleanedIds);
