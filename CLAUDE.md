...
| `dispatch_worker` | Spawn a constrained worker agent via `claude -p` on a separate MCP bucket. Worker loads project context, works within declared scopes (max 3), and returns results including mandatory `stop_scope` output. | Mechanical implementation tasks that don't require user judgment — attribute additions, span additions, pattern application across multiple files/plugins. |
