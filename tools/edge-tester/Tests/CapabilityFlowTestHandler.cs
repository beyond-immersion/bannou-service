...
public class CapabilityFlowTestHandler : IServiceTestHandler
{
    ...
    public ServiceTest[] GetServiceTests()
    {
        return new ServiceTest[]
        {
            ...
            new ServiceTest(TestCapabilityChangeEventsOnSubscription, "Capability - Add/Remove Events", "WebSocket",
                "Test that OnCapabilitiesAdded/Removed events fire when subscribing/unsubscribing")
        };
    }
    ...
    /// <summary>
    /// Tests that OnCapabilitiesAdded/Removed events fire when subscribing/unsubscribing from a game service.
    /// Flow: Connect -> Subscribe to game -> OnCapabilitiesAdded fires with shortcut ->
    ///       Unsubscribe -> OnCapabilitiesRemoved fires with shortcut
    /// </summary>
    private void TestCapabilityChangeEventsOnSubscription(string[] args)
    {
        ...
        Console.WriteLine("Testing that OnCapabilitiesAdded/Removed fire on subscription changes...");

        try
        {
            var result = Task.Run(async () => await PerformCapabilityChangeEventsOnSubscriptionTest()).Result;
            if (result)
            {
                Console.WriteLine("PASSED Capability change events on subscription test");
            }
            else
            {
                Console.WriteLine("FAILED Capability change events on subscription test");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"FAILED Capability change events on subscription test with exception: {ex.Message}");
            if (ex.InnerException != null)
            {
                Console.WriteLine($"   Inner exception: {ex.InnerException.Message}");
            }
    ...
    private async Task<bool> PerformCapabilityChangeEventsOnSubscriptionTest()
    {
        if (Program.Configuration == null)
        {
            Console.WriteLine("   Configuration not available");
        ...
        var openrestyPort = Program.Configuration.OpenRestyPort ?? 80;
        var uniqueId = Guid.NewGuid().ToString("N")[..12];
        var testEmail = $"cap_sub_{uniqueId}@test.local";
        var testPassword = "CapSubTest123!";

        // Step 1: Create/get test-game service
        Console.WriteLine("   Step 1: Getting or creating test-game service...");
        ServiceInfo? service;
        ...
        {
            service = createResponse.Result;
            Console.WriteLine($"   Created service: {service.ServiceId}");
        }
        else if (createResponse.Error?.ResponseCode == 409)
        {
            Console.WriteLine("   Service 'test-game' already exists, fetching...");
            var listResponse = await adminClient.GameService.ListServicesAsync(
                new ListServicesRequest(), timeout: TimeSpan.FromSeconds(5));

            if (!listResponse.IsSuccess || listResponse.Result?.Services == null)
            {
                Console.WriteLine($"   Failed to list services: {listResponse.Error?.Message}");
                return false;
            }

            service = listResponse.Result.Services.FirstOrDefault(s => s.StubName == "test-game");
            if (service == null)
            {
        ...
        {
            Console.WriteLine($"   Failed to create service: {createResponse.Error?.Message}");
            return false;
        }

        // Step 2: Register account and get tokens
        Console.WriteLine("   Step 2: Registering test account...");
        string accessToken;
        string connectUrl;
        Guid accountId;
        ...
        try
        {
            ...
            // Look up account ID
            var accountLookup = await adminClient.Account.GetAccountByEmailAsync(
                new GetAccountByEmailRequest { Email = testEmail },
                timeout: TimeSpan.FromSeconds(5));

            if (!accountLookup.IsSuccess || accountLookup.Result == null)
            {
                Console.WriteLine($"   Failed to look up account: {accountLookup.Error?.Message}");
                return false;
            }

            accountId = accountLookup.Result.AccountId;
        ...
        // Step 3: Connect using BannouClient
        Console.WriteLine("   Step 3: Connecting via BannouClient...");
        ...
        try
        {
            await initialCapabilitiesReceived.Task.WaitAsync(initCts.Token);
            Console.WriteLine("   Initial capabilities received");
        }
        catch (OperationCanceledException)
        {
        ...
        // Step 4: Create subscription for this account
        ...
        {
            Console.WriteLine($"   Failed to create subscription: {subResponse.Error?.Message}");
            return false;
        }

        subscriptionId = subResponse.Result.SubscriptionId;
        Console.WriteLine($"   Created subscription: {subscriptionId}");

        // Step 5: Wait for OnCapabilitiesAdded to fire with the shortcut
        ...
        try
        {
            ...
            if (!hasShortcut)
            {
                Console.WriteLine("   WARNING: Expected shortcut for test-game not found in added capabilities");
                // Don't fail - the shortcut might not be published depending on configuration
            }
        }
        catch (OperationCanceledException)
        {
            Console.WriteLine("   Timeout waiting for OnCapabilitiesAdded after subscription");
            Console.WriteLine("   This may indicate game-session service is not configured to publish shortcuts");
            // Continue to test removal
            added = null;
        }
        ...
        var deleteResponse = await adminClient.Subscription.DeleteSubscriptionAsync(
            new DeleteSubscriptionRequest { SubscriptionId = subscriptionId },
            timeout: TimeSpan.FromSeconds(5));

        if (!deleteResponse.IsSuccess)
        {
            Console.WriteLine($"   Failed to delete subscription: {deleteResponse.Error?.Message}");
            // Continue anyway to check if removal event fires
        }
        else
        {
        ...
        // Step 7: Wait for OnCapabilitiesRemoved to fire
        ...
        catch (OperationCanceledException)
        {
            ...
            // If we at least got the add event, consider it a partial success
            if (added != null && added.Count > 0)
            {
                Console.WriteLine("   OnCapabilitiesAdded worked, but OnCapabilitiesRemoved did not fire");
                Console.WriteLine("   This may indicate the subscription deletion doesn't trigger capability refresh");
                return false;
            }
