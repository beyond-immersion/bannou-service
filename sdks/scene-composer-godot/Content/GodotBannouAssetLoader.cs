...
namespace BeyondImmersion.Bannou.SceneComposer.Godot.Content;
...
/// <remarks>
/// <para>
/// This class bridges the engine-agnostic <see cref="Loaders.IAssetLoader"/> interface
/// to the <see cref="AssetLoader.AssetLoader"/> SDK with Godot type loaders.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public sealed class GodotBannouAssetLoader : Loaders.IAssetLoader, IAsyncDisposable
{
        ...
        Action<string>? debugLog = null)
    {
        ...
        _assetLoader.UseGodot(debugLog);

        Log("GodotBannouAssetLoader initialized with FileSystemAssetSource");
    }
        ...
        Action<string>? debugLog = null)
    {
        ...
        Log("GodotBannouAssetLoader initialized with custom IAssetSource");
    }
    ...
    /// <inheritdoc/>
    public async Task<Mesh?> LoadMeshAsync(
        string bundleId,
        string assetId,
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            var result = await _assetLoader.LoadAssetAsync<Mesh>(assetId, ct).ConfigureAwait(false);
            if (result.Success)
            {
                Log($"Loaded mesh '{assetId}'");
                return result.Asset;
            }

            Log($"Failed to load mesh '{assetId}': {result.ErrorMessage}");
            return null;
        }
        catch (Exception ex)
        {
            Log($"Exception loading mesh '{assetId}': {ex.Message}");
            return null;
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            var result = await _assetLoader.LoadAssetAsync<Texture2D>(assetId, ct).ConfigureAwait(false);
            if (result.Success)
            {
    ...
    /// <inheritdoc/>
    public async Task<AudioStream?> LoadAudioStreamAsync(
        string bundleId,
        string assetId,
        string? variantId = null,
        CancellationToken ct = default)
    {
        ...
        try
        {
            var result = await _assetLoader.LoadAssetAsync<AudioStream>(assetId, ct).ConfigureAwait(false);
            if (result.Success)
            {
                Log($"Loaded audio stream '{assetId}'");
                return result.Asset;
            }

            Log($"Failed to load audio stream '{assetId}': {result.ErrorMessage}");
            return null;
        }
        catch (Exception ex)
        {
            Log($"Exception loading audio stream '{assetId}': {ex.Message}");
            return null;
        }
    }
    ...
    {
        _debugLog?.Invoke($"[GodotBannouAssetLoader] {message}");
    }
