...
namespace BeyondImmersion.Bannou.BehaviorExpressions.Templates;
...
public sealed class AbmlTemplateEngine
{
    ...
    /// <param name="template">The template string.</param>
    /// <param name="scope">Variable scope for template rendering.</param>
    /// <param name="result">The rendered string if successful.</param>
    ...
    private TemplateContext CreateContext(IVariableScope scope)
    {
        ...
        {
            foreach (var (name, value) in varScope.GetLocalVariables())
            {
...
public static class VariableScopeExtensions
{
    ...
    /// <returns>All local variables as key-value pairs.</returns>
