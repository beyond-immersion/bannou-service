...
"""
Generate Guides Catalog from Guide Documents.

Scans docs/guides/*.md and extracts header metadata (Version, Status,
...
def scan_guides(guides_dir: Path) -> list:
    """Scan all guide documents and extract metadata and summaries."""
    guides = []
    ...
    for md_file in sorted(guides_dir.glob('*.md')):
        ...
        key_plugins = extract_header_field(file_lines, 'Key Plugins')
...
def generate_markdown(guides: list) -> str:
    ...
    for guide in guides:
        lines.append(f"## {guide['title']} {{#{guide['anchor']}}}")
        lines.append("")

        # Build metadata line with only non-empty fields
        meta_parts = []
        if guide['version']:
            meta_parts.append(f"**Version**: {guide['version']}")
        if guide['status']:
            meta_parts.append(f"**Status**: {guide['status']}")
    ...
    lines.extend([
        "## Summary",
        "",
        f"- **Documents in catalog**: {len(guides)}",
        "",
        "---",
        "",
...
def main():
    ...
    if not guides:
        print("Warning: No guide documents found")
