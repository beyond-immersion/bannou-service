...
"""
Generate Specifications Catalog from Extension Attribute Specifications.

Scans docs/reference/specifications/*.md and extracts header metadata
(Version, Status, Last Updated, Schema Scope, Generated Output) and
## Summary sections.

Output: docs/generated/GENERATED-SPECIFICATIONS-CATALOG.md

Usage:
    python3 scripts/generate-specifications-catalog.py
"""
...
def scan_specifications(specs_dir: Path) -> list:
    """Scan all specification documents and extract metadata and summaries."""
    docs = []
    ...
    for md_file in sorted(specs_dir.glob('*.md')):
        ...
        version = extract_header_field(file_lines, 'Version')
        status = extract_header_field(file_lines, 'Status')
        last_updated = extract_header_field(file_lines, 'Last Updated')
        schema_scope = extract_header_field(file_lines, 'Schema Scope')
        generated_output = extract_header_field(file_lines, 'Generated Output')
...
def generate_markdown(docs: list) -> str:
    ...
    for doc in docs:
        ...
        meta_parts = []
        if doc['version']:
            meta_parts.append(f"**Version**: {doc['version']}")
        if doc['status']:
            meta_parts.append(f"**Status**: {doc['status']}")
        if doc['last_updated']:
            meta_parts.append(f"**Last Updated**: {doc['last_updated']}")
        if doc['schema_scope']:
            meta_parts.append(f"**Schema Scope**: {doc['schema_scope']}")
        meta_parts.append(f"[Full Specification](../reference/specifications/{doc['filename']})")
    ...
    lines.extend([
        "## Summary",
        "",
        f"- **Specifications in catalog**: {len(docs)}",
        "",
        "---",
        "",
...
def main():
    ...
    specs_dir = repo_root / 'docs' / 'reference' / 'specifications'

    docs = scan_specifications(specs_dir)

    if not docs:
        print("Warning: No specification documents found")

    markdown = generate_markdown(docs)

    output_file = repo_root / 'docs' / 'generated' / 'GENERATED-SPECIFICATIONS-CATALOG.md'
    with open(output_file, 'w') as f:
        f.write(markdown)

    print(f"Generated {output_file} with {len(docs)} specifications")
