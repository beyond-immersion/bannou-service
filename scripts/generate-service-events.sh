...
for EVENTS_SCHEMA in ../schemas/*-events.yaml; do
    ...
    if [ -f "$RESOLVED_SCHEMA" ]; then
        RESOLVED_COMMON_REFS=$(extract_common_api_refs "$RESOLVED_SCHEMA")
        if [ -n "$RESOLVED_COMMON_REFS" ]; then
            if [ -n "$COMMON_REFS" ]; then
                COMMON_REFS=$(echo "${COMMON_REFS},${RESOLVED_COMMON_REFS}" | tr ',' '\n' | sort -u | tr '\n' ',' | sed 's/,$//')
            else
                COMMON_REFS="$RESOLVED_COMMON_REFS"
            fi
        fi
    fi
