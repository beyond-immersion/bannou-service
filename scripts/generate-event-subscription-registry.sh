...
# Scans all *-service-events.yaml files and generates EventSubscriptionRegistration.cs
...
echo -e "${BLUE}📡 Generating EventSubscriptionRegistration from all *-service-events.yaml files...${NC}"
...
# Find all *-service-events.yaml files (excluding client events)
for yaml_file in sorted(Path(schemas_dir).glob("*-service-events.yaml")):
...
echo -e "  📊 Found $SUBSCRIPTION_COUNT event subscription(s) across all services"
...
echo -e "  📝 Registered $SUBSCRIPTION_COUNT topic→event mappings"
