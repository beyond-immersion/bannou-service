...
# Extract inlined type names from a resolved event schema
# The resolver inlines complex types that had nested refs - these should be excluded
# since they're already generated by the service's API schema
# Usage: INLINED_TYPES=$(extract_inlined_types "$resolved_schema")
# Returns: Comma-separated list of inlined type names (types not ending in Event or Command)
extract_inlined_types() {
    local resolved_schema="$1"
    ...
    # Get schema type names from the components/schemas section
    # Use awk to only process lines after "schemas:" until next top-level key
    # Pattern: matches "    TypeName:" (4 spaces, PascalCase, colon)
    # Note: Use positive match with 'next' to skip Event/Command types
    # because bash escaping causes issues with the !~ operator
    local inlined_types=$(awk '
        /^components:/ { in_components = 1; next }
        /^  schemas:/ && in_components { in_schemas = 1; next }
        /^  [a-z]/ && in_components { in_schemas = 0 }
        /^[a-z]/ { in_components = 0; in_schemas = 0 }
        in_schemas && /^    [A-Z][a-zA-Z0-9]+:/ {
            name = $0
            gsub(/:.*/, "", name)
            gsub(/^ +/, "", name)
            # Skip Event and Command types (those are native to the events schema)
            if (name ~ /(Event|Command)$/) next
            print name
        }
    ' "$resolved_schema" | sort -u | tr '\n' ',' | sed 's/,$//')
