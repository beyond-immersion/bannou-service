...
# Extract $refs to common-api.yaml types (shared types like EntityType, InstanceHealthStatus)
# These are already generated by generate-common-api.sh into CommonApiModels.cs
# and must be excluded to prevent duplicate type definitions across namespaces
COMMON_REFS=$(extract_common_api_refs "$COMMON_EVENTS_SCHEMA")
...
if [ -n "$COMMON_REFS" ]; then
    ...
    # Common API types are in BeyondImmersion.BannouService namespace
    NAMESPACE_USAGES="${NAMESPACE_USAGES},BeyondImmersion.BannouService"
    echo -e "  ${BLUE}Excluding common API types: ${COMMON_REFS}${NC}"
fi
