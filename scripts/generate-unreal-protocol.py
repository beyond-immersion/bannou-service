...
"""
Generate Unreal Engine C++ protocol headers from Bannou binary protocol specification.

This script generates header-only C++ files compatible with UE4/UE5:
- BannouProtocol.h: Binary protocol constants, message flags, response codes
- Network byte order utilities for big-endian serialization
- RFC 4122 GUID serialization matching the TypeScript SDK

Output:
- sdks/unreal/Generated/BannouProtocol.h

Usage:
    python3 scripts/generate-unreal-protocol.py
"""

import sys
from pathlib import Path

# Protocol constants (must match TypeScript SDK)
REQUEST_HEADER_SIZE = 31
RESPONSE_HEADER_SIZE = 16

# Message flags (from MessageFlags.ts)
MESSAGE_FLAGS = {
    'None': (0x00, 'Default - JSON text, service request, unencrypted, standard priority, expects response'),
    'Binary': (0x01, 'Message payload is binary data (not JSON)'),
    'Encrypted': (0x02, 'Message payload is encrypted'),
    'Compressed': (0x04, 'Message payload is compressed (gzip)'),
    'HighPriority': (0x08, 'Deliver at high priority, skip to front of queues'),
    'Event': (0x10, 'Fire-and-forget message, no response expected'),
    'Client': (0x20, 'Route to another WebSocket client (not a Bannou service)'),
    'Response': (0x40, 'Message is a response to an RPC (not a new request)'),
    'Meta': (0x80, 'Request metadata about endpoint instead of executing it'),
}

# Response codes (from ResponseCodes.ts)
RESPONSE_CODES = {
    # Protocol-level (0-49)
    'OK': (0, 'Request completed successfully'),
    'RequestError': (10, 'Generic request error (malformed message, invalid format)'),
    'RequestTooLarge': (11, 'Request payload exceeds maximum allowed size'),
    'TooManyRequests': (12, 'Rate limit exceeded for this client/session'),
    'InvalidRequestChannel': (13, 'Invalid channel number in request header'),
    'Unauthorized': (20, 'Authentication required but not provided or invalid'),
    'ServiceNotFound': (30, 'Target service GUID not found in capability manifest'),
    'ClientNotFound': (31, 'Target client GUID not found (for client-to-client messages)'),
    'MessageNotFound': (32, 'Referenced message ID not found'),
    'BroadcastNotAllowed': (40, 'Broadcast not allowed in this connection mode'),
    # Service-level (50-69)
    'Service_BadRequest': (50, 'Service returned 400 Bad Request'),
    'Service_NotFound': (51, 'Service returned 404 Not Found'),
    'Service_Unauthorized': (52, 'Service returned 401/403 Unauthorized/Forbidden'),
    'Service_Conflict': (53, 'Service returned 409 Conflict'),
    'Service_InternalServerError': (60, 'Service returned 500 Internal Server Error'),
    # Shortcut-specific (70+)
    'ShortcutExpired': (70, 'Shortcut has expired and is no longer valid'),
    'ShortcutTargetNotFound': (71, 'Shortcut target endpoint no longer exists'),
    'ShortcutRevoked': (72, 'Shortcut was explicitly revoked'),
}


def generate_protocol_header() -> str:
    """Generate the BannouProtocol.h header file."""
    header = '''// <auto-generated>
// BannouProtocol.h - Binary protocol constants and utilities for Bannou WebSocket API
// Generated by generate-unreal-protocol.py. Do not edit this file manually.
// </auto-generated>
...
{{
     ...
     */
    constexpr int32 REQUEST_HEADER_SIZE = {REQUEST_HEADER_SIZE};
     ...
     */
    constexpr int32 RESPONSE_HEADER_SIZE = {RESPONSE_HEADER_SIZE};
...
'''
    ...
    # Add message flag enum values
    for name, (value, description) in MESSAGE_FLAGS.items():
        header += f'        /** {description} */\n'
        header += f'        {name} = 0x{value:02X},\n\n'
...
'''
    ...
    # Add response code enum values
    for name, (value, description) in RESPONSE_CODES.items():
        header += f'        /** {description} */\n'
        header += f'        {name} = {value},\n\n'
...
'''
    ...
    # Add response code name switch cases
    for name, (value, _) in RESPONSE_CODES.items():
        header += f'            case EResponseCode::{name}: return TEXT("{name}");\n'

    header += '''            default: return TEXT("UnknownError");
        }
    }
...
def main():
    """Generate the Unreal protocol header."""
    ...
    print("Generating Unreal Engine protocol header...")

    header_content = generate_protocol_header()

    output_path = output_dir / 'BannouProtocol.h'
    with open(output_path, 'w', newline='\n') as f:
        f.write(header_content)

    print(f"  Generated {output_path}")
    print("Protocol header generation complete.")
