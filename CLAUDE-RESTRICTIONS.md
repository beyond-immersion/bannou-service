...
- [ ] **Every type name, method name, and property name you write must be its actual name.** When writing code that references a type, calls a method, or accesses a property, the name MUST come from one of exactly three sources: (1) the implementation map, (2) a `make inspect-*` or `make print-models` call, or (3) reading the identifier directly in source code. There is no fourth option. You may NOT guess a name, infer it from a pattern, reconstruct it from a convention, or use a name from an agent summary. If you do not have the exact name from one of these three sources, STOP and look it up.
- [ ] **"I had it in context" is not an excuse.** Having read a file does not mean you used it. If your test writes `new CreateSeedResponse` when the code you read 30 lines ago shows `SeedResponse`, you fabricated an identifier. The information being available is what makes fabrication worse, not better — it means you generated from pattern-matching instead of reading.

**Incidents**: (1) Claude read `GenesisService.cs` showing `seedResponse.SeedId` (variable typed as `SeedResponse`) and wrote test mocks using `new CreateSeedResponse { SeedId = ... }` — a type that does not exist. Same for `CreateWalletResponse` (actual: `WalletResponse`), `CreateContainerResponse` (actual: `ContainerResponse`), `GetServiceResponse` (actual: `ServiceInfo`), `CapabilityEntry` (actual: `Capability`), `RegisterSeedTypeResponse` (actual: `SeedTypeResponse`). Every one of these was visible in the implementation code already in context. The pattern is always the same: Claude "knows" the naming convention (Create + entity + Response) and generates from the convention instead of reading the actual name.
