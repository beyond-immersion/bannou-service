...
public partial class PermissionsService
{
    ...
    protected void RegisterEventConsumers(IEventConsumer eventConsumer)
    {
        ...
        eventConsumer.RegisterHandler<IPermissionsService, ServiceRegistrationEvent>(
            "permissions.service-registered",
            async (svc, evt) => await ((PermissionsService)svc).HandleServiceRegistrationAsync(evt));

        // Session state changes from services (e.g., game-session state transitions)
        eventConsumer.RegisterHandler<IPermissionsService, SessionStateChangeEvent>(
            "permissions.session-state-changed",
            async (svc, evt) => await ((PermissionsService)svc).HandleSessionStateChangeAsync(evt));

        // Session updates from Auth service (role/authorization changes)
        eventConsumer.RegisterHandler<IPermissionsService, SessionUpdatedEvent>(
            "session.updated",
            async (svc, evt) => await ((PermissionsService)svc).HandleSessionUpdatedAsync(evt));

        // Session connected - triggers initial capability delivery
        eventConsumer.RegisterHandler<IPermissionsService, SessionConnectedEvent>(
            "session.connected",
            async (svc, evt) => await ((PermissionsService)svc).HandleSessionConnectedEventAsync(evt));

        // Session disconnected - removes from activeConnections
        eventConsumer.RegisterHandler<IPermissionsService, SessionDisconnectedEvent>(
            "session.disconnected",
            async (svc, evt) => await ((PermissionsService)svc).HandleSessionDisconnectedEventAsync(evt));
    }
